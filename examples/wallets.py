...
pubkey = (
    "tpubDDkyPBhSAx8DFYxx5aLjvKH6B6Eq2eDK1YN76x1WeijE8eVUswpibGbv8zJjD6yLDHzVcqWzSp2fWVFhEW9XnBssFqM"
    "wt9SrsVeBeqfBbR3"
)
...
litecoin_wallet = Wallet.create(
    db_uri=test_database, name="Litecoin Wallet", network="litecoin"
)
...
print("\n=== Create Dash wallet ===")
dash_wallet = Wallet.create(db_uri=test_database, name="Dash Wallet", network="dash")
dash_wallet.new_key()
...
words = (
    "blind frequent camera goddess pottery repair skull year mistake wrist lonely mix"
)
...
w = Wallet.create(name="Wallet Error", db_uri=test_database)
try:
    w.import_key(
        key="T43gB4F6k1Ly3YWbMuddq13xLb56hevUDP3RthKArr7FPHjQiXpp", network="litecoin"
    )
...
w = Wallet.create(
    "P2SH-segwit-Wallet", witness_type="p2sh-segwit", db_uri=test_database
)
w.new_key()
w.new_key(network="litecoin")
w.info()
