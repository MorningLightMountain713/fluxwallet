...
class TestMnemonics(unittest.TestCase):
    ...
    def test_mnemonic_generate_error(self):
        self.assertRaisesRegexp(
            ValueError, "Strength should be divisible by 32", Mnemonic().generate, 11
        )

    def test_mnemonic_to_entropy(self):
        phrase = (
            "usage grid neither voice worry armor sudden core excuse keen stand pudding"
        )
        self.assertEqual(
            Mnemonic().to_entropy(phrase),
    ...
    def test_mnemonic_to_mnemonic(self):
        ...
        self.assertEqual(
            Mnemonic().to_mnemonic(
                "28acfc94465fd2f6774759d6897ec122", add_checksum=False
            ),
            "action filter venture match garlic nut oven modify output dwarf wild cattle",
        )
        self.assertRaisesRegexp(
            ValueError,
            "Integer value of data should be in secp256k1 domain between 1 and "
            "secp256k1_n-1",
            Mnemonic().to_mnemonic,
    ...
    def test_mnemonic_to_seed_invalid_checksum(self):
        phrase = (
            "runway truly foil future recall scatter garage over floor clutch shy boat"
        )
        self.assertRaisesRegexp(
            ValueError, "Invalid checksum 0110 for entropy", Mnemonic().to_seed, phrase
        )

    def test_mnemonic_exceptions(self):
        self.assertRaisesRegexp(
            ValueError, "Strength should be divisible by 32", Mnemonic().generate, 20
        )
        self.assertRaisesRegexp(
            ValueError,
