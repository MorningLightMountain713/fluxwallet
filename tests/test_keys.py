...
from fluxwallet.networks import NETWORK_DEFINITIONS
...
class TestKeyClasses(unittest.TestCase):
    def test_keys_classes_dunder_methods(self):
        pk = (
            "xprv9s21ZrQH143K4EDmQNMBqXwUTcrRoUctKkTegGsaBcMLnR1fJkMjVSRwVswjHzJspfWCUwzge1F521cY4wfWD54tzXVUqeo"
            "TFkZo17HiK2y"
        )
        k = HDKey(pk)
        self.assertEqual(
        ...
        self.assertEqual(
            int(k),
            95796105828208927954168018443072630832764875640480247096632116413925408206516,
        )
        k2 = HDKey(pk)
        ...
        self.assertEqual(
            str(pubk2),
            "03dc86716b2be27a0575558bac73279290ac22c3ea0240e42a2152d584f2b4006b",
        )
        self.assertTrue(k.public() == pubk2)
        self.assertEqual(
            k + k2,
            b"\x03\xdc\x86qk+\xe2z\x05uU\x8b\xacs'\x92\x90\xac\"\xc3\xea\x02@\xe4*!R\xd5"
    ...
    def test_path_expand(self):
        ...
        self.assertListEqual(
            path_expand([10, 20]), ["m", "44'", "0'", "0'", "10", "20"]
        )
        self.assertListEqual(
        ...
        self.assertListEqual(
            path_expand([], witness_type="p2sh-segwit"),
            ["m", "49'", "0'", "0'", "0", "0"],
        )
        self.assertListEqual(
            path_expand([99], witness_type="p2sh-segwit", multisig=True),
            ["m", "48'", "0'", "0'", "1'", "0", "99"],
        )
        self.assertRaisesRegexp(
            BKeyError,
            "Invalid path provided. Path should be shorter than 6 items.",
            path_expand,
            [0, 1, 2, 3, 4, 5, 6],
        )
        self.assertRaisesRegexp(
            BKeyError,
            "Please provide path as list with at least 1 item",
            path_expand,
            5,
        )
...
class TestGetKeyFormat(unittest.TestCase):
    ...
    def test_format_bin_uncompressed(self):
        key = (
            b"\x04\xa8\x82\xd4\x14\xe4x\x03\x9c\xd5\xb5*\x92\xff\xb1=\xd5\xe6\xbdE\x15It9\xdf\xfdi\x1a\x0f\x12\xaf"
    ...
    def test_format_hdkey_private(self):
        key = (
            "xprv9s21ZrQH143K2JF8RafpqtKiTbsbaxEeUaMnNHsm5o6wCW3z8ySyH4UxFVSfZ8n7ESu7fgir8imbZKLYVBxFPND1pniTZ81v"
            "Kfd45EHKX73"
        )
    ...
    def test_format_hdkey_public(self):
        key = (
            "xpub6AHA9hZDN11k2ijHMeS5QqHx2KP9aMBRhTDqANMnwVtdyw2TDYRmF8PjpvwUFcL1Et8Hj59S3gTSMcUQ5gAqTz3Wd8EsMTmF3"
            "DChhqPQBnU"
        )
    ...
    def test_format_hdkey_private_litecoin(self):
        key = (
            "ttpv96BtqegdxXceS41gmguxRWjtFdG11XqquvaPcpionXB8oQ2zkXYY9vCvykdfLEeeihx1SxP3t7ranmqLHKbmgfiMFR7967urhq"
            "oAq5eBHur"
        )
        self.assertEqual("hdkey_private", get_key_format(key)["format"])
    ...
    def test_format_key_exceptions(self):
        self.assertRaisesRegexp(
            BKeyError, "Key empty, please specify a valid key", get_key_format, ""
        )
        self.assertRaisesRegexp(
            BKeyError,
            "Attribute 'is_private' must be False or True",
            get_key_format,
            "666368e477a8ddd46808c527cc3c506719bb3f52a927b6c13532984b714b56ad",
            3,
        )
...
class TestPrivateKeyConversions(unittest.TestCase):
    ...
    def test_private_key_conversions_dec(self):
        self.assertEqual(
            83827997552125623280808720137320612316470870230953489181279239295529837939288,
            self.k.secret,
        )
    ...
    def test_private_key_conversions_wif_uncompressed(self):
        self.assertEqual(
            "5KDudqswBNJ8mf2k7Gxn72UknDBh7GFjj9NGJrY22SY1hjKS1gF", self.ku.wif()
        )

    def test_private_key_conversions_wif(self):
        self.assertEqual(
            "L3RyKcjp8kzdJ6rhGhTC5bXWEYnC2eL3b1vrZoduXMht6m9MQeHy", self.k.wif()
        )
        self.assertEqual(
            "XHVtmt8BSSd5MRs5JTT4apiX9a3mUSwHxbGm6Ky6qiyyVvFRhmU7",
            self.k.wif(prefix="cc"),
        )
        self.assertEqual(
            "XHVtmt8BSSd5MRs5JTT4apiX9a3mUSwHxbGm6Ky6qiyyVvFRhmU7",
            self.k.wif(prefix=b"\xcc"),
        )
    ...
    def test_private_key_public_uncompressed(self):
        self.assertEqual(
            "044781e448a7ff0e1b66f1a249b4c952dae33326cf57c0a643738886f4efcd14d57a380bc32c26f46e733c"
            "d991064c2e7f7d532b9c9ca825671a8809ab6876c78b",
            self.ku.public_uncompressed_hex,
        )
...
class TestPrivateKeyImport(unittest.TestCase):
    def test_private_key_import_key(self):
        self.k = Key(
            61876261089097932796193024729035977913579848833009517639587741086858579422075
        )
        self.assertEqual(
            "L1odb1uUozbfK2NrsMyhJfvRsxGM2AxixgPL8vG9BUBnE6W1VyTX", self.k.wif()
        )
    ...
    def test_private_key_import_key_hex_compressed(self):
        ...
        self.assertEqual(
            "KxFC1jmwwCoACiCAWZ3eXa96mBM6tb3TYzGmf6YwgdGWZgawvrtJ", self.k.wif()
        )
    ...
    def test_private_key_import_key_byte(self):
        ...
        self.k = Key(pk)
        self.assertEqual(
            "KyBsPXxTuVD82av65KZkrGrWi5qLMah5SdNq6uftawDbgKa2wv6S", self.k.wif()
        )

    def test_private_key_import_hex(self):
        pk = (
            "4781e448a7ff0e1b66f1a249b4c952dae33326cf57c0a643738886f4efcd14d57a380bc32c26f46e733cd"
    ...
    def test_private_key_import_wif(self):
        ...
        self.assertEqual(
            "88ccb90221d9b44df8dd317307de2d6019c9c7448dccaa1e45bae77e5a022b7b",
            self.k.private_hex,
        )
    ...
    def test_private_key_import_error_1(self):
        self.assertRaisesRegexp(
            BKeyError,
            "Invalid checksum, not a valid WIF key",
            Key,
            "L1odb1uUozbfK2NrsMyhJfvRsxGM2axixgPL8vG9BUBnE6W1VyTX",
        )
    ...
    def test_private_key_import_error_2(self):
        self.assertRaisesRegexp(
            ...
            Key,
            "M1odb1uUozbfK2NrsMyhJfvRsxGM2AxixgPL8vG9BUBnE6W1VyTX",
        )

    def test_private_key_import_testnet(self):
        self.k = Key("92Pg46rUhgTT7romnV7iGW6W1gbGdeezqdbJCzShkCsYNzyyNcc", "testnet")
        self.assertEqual(
            "92Pg46rUhgTT7romnV7iGW6W1gbGdeezqdbJCzShkCsYNzyyNcc", self.k.wif()
        )
    ...
    def test_private_key_import(self):
        ...
        self.assertEqual(
            Key.from_wif(wif).address(), "1Nro9WkpaKm9axmcfPVp79dAJU1Gx7VmMZ"
        )
        wif = "L1NUEdnjKvK547BrFjRnuSvyLn2Ndkyjz9gQwc9dTF9ucRDV7SsP"
        self.assertEqual(
            Key.from_wif(wif).address(), "1LuhZSrbPrd45RLa2EZ2JnrA4NtLf6sb58"
        )
        ...
        self.assertFalse(Key.from_wif(wif).compressed)
        self.assertEqual(
            Key.from_wif(wif).address(), "17VZNX1SN5NtKa8UQFxwQbFeFc3iqRYhem"
        )
...
class TestPublicKeyConversion(unittest.TestCase):
    def setUp(self):
        self.publickey_hex = (
            "043be860d524b1aef015d0e501333d5e62fd39652bfb89e6720c3eb1cb10754370eeee80b04abf2f80dec"
    ...
    def test_public_key_get_address_uncompressed(self):
        self.assertEqual(
            "1BwbZw1fG91jMYsXh6hfnvPGXBaMcughNL", self.K.address_uncompressed()
        )
    ...
    def test_public_key_get_point(self):
        self.assertEqual(
            (
                27097034899423571266687886742220335326047800315940073646681141977460289913712,
    ...
    def test_public_key_get_hash160_uncompressed(self):
        self.assertEqual(
            "78049354383f043fb15a04be58a289ef8a2c03fa", self.K.hash160.hex()
        )

    def test_public_key_get_hash160(self):
        self.assertEqual(
            "f19c417fd97e364afb06e1edd2c0e6a7ecf1af00", self.KC.hash160.hex()
        )
    ...
    def test_public_key_import_error(self):
        self.assertRaisesRegexp(
            ...
            "Unrecognised key format",
            Key,
            "064781e448a7ff0e1b66f1a249b4c952dae33326cf57c0a643738886f4efcd14d5",
        )

    def test_litecoin_private_key(self):
        KC_LTC = Key(
            "0bc295d0b20b0e2ff6ab2c4982583d4f84936a17689aaca031a803dcf4a3b139",
            network="litecoin",
        )
        self.assertEqual(
            KC_LTC.wif(), "T3SqWmDzttRHnfypMorvRgPpG48UH1ZE7apvoLUGTDidKtf3Ts2u"
        )
        self.assertEqual(KC_LTC.address(), "LeA97dLDPrjRsPhwrQJxUWJUPErGo516Ct")
        self.assertEqual(
            KC_LTC.public_hex,
            "02967b4671563ceeab16f22c36605d97fbf254fadba0fa48f75c03f27d11584f92",
        )
...
class TestPublicKeyUncompressed(unittest.TestCase):
    ...
    def test_public_key_address_uncompressed(self):
        self.assertEqual(
            "1thMirt546nngXqyPEz532S8fLwbozud8", self.K.address_uncompressed()
        )
...
class TestHDKeysImport(unittest.TestCase):
    def setUp(self):
        ...
        self.k2 = HDKey.from_seed(
            "fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a878"
            "4817e7b7875726f6c696663605d5a5754514e4b484542"
        )
        self.xpub = HDKey(
            "xpub661MyMwAqRbcFtXgS5sYJABqqG9YLmC4Q1Rdap9gSE8NqtwybGhePY2gZ29ESFjqJoCu1Rupje8YtGqse"
    ...
    def test_hdkey_import_seed_2(self):
        self.assertEqual(
            "xprv9s21ZrQH143K31xYSDQpPDxsXRTUcvj2iNHm5NUtrGiGG5e2DtALGdso3pGz6ssrdK4PFmM8NSpSBHNqPqm55Qn3L"
            "qFtT2emdEXVYsCzC2U",
            self.k2.wif(is_private=True),
        )
        self.assertEqual(
    ...
    def test_hdkey_import_extended_private_key(self):
        extkey = (
            "xprv9z4pot5VBttmtdRTWfWQmoH1taj2axGVzFqSb8C9xaxKymcFzXBDptWmT7FwuEzG3ryjH4ktypQSAewRiNMjANTtpgP4m"
            "LTj34bhnZX7UiM"
        )
        self.k = HDKey(extkey)
    ...
    def test_hdkey_import_extended_public_key(self):
        extkey = (
            "xpub6D4BDPcP2GT577Vvch3R8wDkScZWzQzMMUm3PWbmWvVJrZwQY4VUNgqFJPMM3No2dFDFGTsxxpG5uJh7n7epu4trkrX7x7"
            "DogT5Uv6fcLW5"
        )
        self.k = HDKey(extkey)
    ...
    def test_hdkey_import_bip38_key(self):
        if USING_MODULE_SCRYPT:
            ...
            self.assertEqual(
                "L44B5gGEpqEDRS9vVPz7QT35jcBG2r3CZwSwQ4fCewXAhAhqGVpP", self.k.wif_key()
            )
    ...
    def test_hdkey_import_from_private_byte(self):
        keystr = (
            b"fch\xe4w\xa8\xdd\xd4h\x08\xc5'\xcc<Pg\x19\xbb?R\xa9'\xb6\xc152\x98KqKV\xad\x91`G-a\xb1\xad\xd8eL"
    ...
    def test_hdkey_import_private_uncompressed(self):
        ...
        self.assertEqual(
            k.private_hex,
            "427ea8c9c286fc050b8ccd8d8a7a8de57322c519ba79b3fad745bc4e97ed5a37",
        )
        self.assertEqual(
            k.public_hex,
            "0403c6eb8873620ac55a6035b0fd527e623f7659c97f30cb8d6a3fe90e924da5fa197aa4"
        ...
        self.assertEqual(
            k3.public_hex,
            "0303c6eb8873620ac55a6035b0fd527e623f7659c97f30cb8d6a3fe90e924da5fa",
        )
...
class TestHDKeysChildKeyDerivation(unittest.TestCase):
    def setUp(self):
        self.k = HDKey.from_seed("000102030405060708090a0b0c0d0e0f")
        self.k2 = HDKey.from_seed(
            "fffcf9f6f3f0edeae7e4e1dedbd8d5d2cfccc9c6c3c0bdbab7b4b1aeaba8a5a29f9c999693908d8a8"
    ...
    def test_hdkey_path_m_0h(self):
        ...
        self.assertEqual(
            "xpub68Gmy5EdvgibQVfPdqkBBCHxA5htiqg55crXYuXoQRKfDBFA1WEjWgP6LHhwBZeNK1VTsfTFUHCdrfp1bgwQ9x"
    ...
    def test_hdkey_path_m_0h_1(self):
        ...
        self.assertEqual(
            "xpub6ASuArnXKPbfEwhqN6e3mwBcDTgzisQN1wXN9BJcM47sSikHjJf3UFHKkNAWbWMiGj7Wf5uMash7SyYq527Hqc"
            "k2AxYysAA7xmALppuCkwQ",
            sk.wif(),
        )
    ...
    def test_hdkey_path_key2(self):
        ...
        self.assertEqual(
            "xprvA2nrNbFZABcdryreWet9Ea4LvTJcGsqrMzxHx98MMrotbir7yrKCEXw7nadnHM8Dq38EGfSh6dqA9QWTyefMLEc"
            "BYJUuekgW4BYPJcr9E7j",
            sk.wif(is_private=True),
        )
        self.assertEqual(
            "xpub6FnCn6nSzZAw5Tw7cgR9bi15UV96gLZhjDstkXXxvCLsUXBGXPdSnLFbdpq8p9HmGsApME5hQTZ3emM2rnY5agb"
    ...
    def test_hdkey_path_M_0_1_public(self):
        ...
        self.assertEqual(
            "xpub6AvUGrnEpfvJBbfx7sQ89Q8hEMPM65UteqEX4yUbUiES2jHfjexmfJoxCGSwFMZiPBaKQT1RiKWrKfuDV4vpgVs"
            "4Xn8PpPTR2i79rwHd4Zr",
            sk.wif(),
        )
    ...
    def test_hdkey_bip44_account(self):
        pk = (
            "tprv8ZgxMBicQKsPdvHCP6VxtFgowj2k7nBJnuRiVWE4DReDFojkLjyqdT8mtR6XJK9dRBcaa3RwvqiKFjsEQVhKfQmHZCCYf4jRTWv"
            "JuVuK67n"
        )
        k = HDKey(pk)
        self.assertEqual(
            k.public_master(3, 45, as_private=True).private_hex,
            "232b9d7b48fa4ca6e842f09f6811ff03cf33ba0582b4cca5752deec2e746c186",
        )

    def test_hdkey_bip44_account_litecoin(self):
        pk = (
            "Ltpv71G8qDifUiNes8hK1m3ZjL3bW76X4AKF3J26FVDM5awe6mWdyyzZgTrbvkK5z4WQyKkyVnDvC56KfRaHHhcZjWcWvRFCzBYUsCc"
            "FoNNHjck"
        )
        k = HDKey(pk, network="litecoin")
        self.assertEqual(
            k.public_master().address(), "LZ4gg2m6uNY3vj9RUFkderRP18ChTwWyiq"
        )

    def test_hdkey_derive_from_public_error(self):
        k = HDKey().public()
        self.assertRaisesRegexp(
            BKeyError, "Need a private key to create child private key", k.child_private
        )
        k0 = HDKey()
        k1 = k0.child_private(10, hardened=True)
        self.assertRaisesRegexp(
            BKeyError,
            "Cannot derive hardened key from public private key",
            k1.child_public,
            2147483659,
        )
...
class TestHDKeysPublicChildKeyDerivation(unittest.TestCase):
    def setUp(self):
        self.K = HDKey(
            "xpub6ASuArnXKPbfEVRpCesNx4P939HDXENHkksgxsVG1yNp9958A33qYoPiTN9QrJmWFa2jNLdK84bWmyqTSPGtApP8P"
            "7nHUYwxHPhqmzUyeFG"
        )
        self.k = HDKey(
    ...
    def test_hdkey_derive_public_child_key(self):
        self.assertEqual(
            "1BvgsfsZQVtkLS69NvGF8rw6NZW2ShJQHr", self.K.child_public(0).address()
        )

    def test_hdkey_derive_public_child_key2(self):
        self.assertEqual(
            "17JbSP83rPWmbdcdtiiTNqBE8MgGN8kmUk", self.K.child_public(8).address()
        )
    ...
    def test_hdkey_private_key_hardened(self):
        ...
        self.assertEqual(
            "xprv9wTErTSu5AWGkDeUPmqBcbZWX1xq85ZNX9iQRQW9DXwygFp7iRGJo79dsVctcsCHsnZ3XU3DhsuaGZbDh8iDkB"
            "N45k67UKsJUXM1JfRCdn1",
            str(self.k2.subkey_for_path("3/2H").wif(is_private=True)),
        )

    def test_hdkey_litecoin(self):
        k = HDKey(
            "Ltpv71G8qDifUiNetj2H4no6Q4oB8o2eUH8tSU2BsJDGyKTyMJ6ejPDXHWtQeTzKQdEeEexxyw3vSAYtxnAz3qYZc"
            "59jfTiqHLzjKkwJ9iDJ1uC",
            network="litecoin",
        )
        self.assertEqual(
            "LWsiwZnGg74CFHEaLPzfASxktrzDYYSwvM", k.child_public(0).address()
        )
        self.assertEqual(
            "LfH72Fgeikvhu1y5rtMAkQ5SS5aJJUafLX", k.child_public(100).address()
        )
        self.assertEqual(
            "T65a5dNtdayWp9F638f8fokiyixCA4fhyzb7FWFYXjejqjaxKRSc",
            k.child_private(6).wif_key(),
        )
        self.assertEqual(
            "Ltpv75tiiksDF3fUqK8jkAfwY1h3zDLs3oCFQa5wXDNh981n6LDJZ6juFWUJwwkN3pKbr3diSdMkZfYAhwhkhjP9qG"
            "wviSbMXtEJYxoH2m3FbDQ",
            str(k.subkey_for_path("3H/1").wif(is_private=True)),
        )
...
class TestHDKeys(unittest.TestCase):
    ...
    def test_hdkey_testnet_import(self):
        self.k = HDKey(
            "tprv8ZgxMBicQKsPf2S18qpSypHPZBK7mdiwvXHPh5TSjGjm2pLacP4tEqVjLVyagTLLgSZK4YyBNb4eytBykE755Kc"
            "L9YXAqPtfERNRfwRt54M"
        )

        self.assertEqual(
            "cPSokRrLueavzAmVBmAXwgALkumRNMN9pErvRLAXvx58NBJAkEYJ", self.k.wif_key()
        )
        self.assertEqual(
            "tpubD6NzVbkrYhZ4YVTo2VV3PDwW8Cq3vxurVptAybVk9YY9sJbMEmtURL7bWgKxXSWSahXu6HbHkdpjBGzwYYkJm"
            "u2VmoeHuiTmzHZpJo8Cdpb",
            self.k.wif_public(),
        )
    ...
    def test_hdkey_uncompressed_key_conversion(self):
        ...
        self.assertEqual(
            hdkey_compressed.wif_key(),
            "KyD9aZEG9cHZa3Hnh3rnTAUHAs6XhroYtJQwuBy4qfBhzHGEApgv",
        )
...
class TestKeysBulk(unittest.TestCase):
    ...
    def test_hdkey_derive_from_public_and_private_index(self):
        ...
        for i in range(BULKTESTCOUNT):
            ...
            if pub_with_privparent != pub_with_pubparent:
                print(
                    "Error index %4d: pub-child %s, priv-child %s"
    ...
    def test_hdkey_derive_from_public_and_private_random(self):
        ...
        for i in range(BULKTESTCOUNT):
            ...
            if pub_with_privparent != pub_with_pubparent:
                print(
                    "Error random key: %4d: pub-child %s, priv-child %s"
                    % (i, pub_with_privparent, pub_with_pubparent)
                )
            self.assertEqual(pub_with_pubparent, pub_with_privparent)
...
class TestKeysAddress(unittest.TestCase):
    ...
    def test_keys_address_import_conversion(self):
        ...
        ac = Address.import_address(
            address_legacy, network_overrides={"prefix_address_p2sh": "32"}
        )
    ...
    def test_keys_address_deserialize_exceptions(self):
        self.assertRaisesRegexp(
            BKeyError,
            "Invalid address 17N9VQbP89ThunSq7Yo2VooXCFTW1Lp8bb, checksum incorrect",
            deserialize_address,
            "17N9VQbP89ThunSq7Yo2VooXCFTW1Lp8bb",
            encoding="base58",
        )
        self.assertRaisesRegexp(
            EncodingError,
            ...
            deserialize_address,
            "17N9VQbP89ThunSq7Yo2VooXCFTW1Lp8bd",
            encoding="bs",
        )
        self.assertRaisesRegexp(
            EncodingError,
        ...
        self.assertRaisesRegexp(
            ...
            "encoding base58",
            deserialize_address,
            "bc1qk077yl8zf6yty25rgrys8h40j8adun267y3m44",
            encoding="base58",
        )
    ...
    def test_keys_address_deserialize_bech32(self):
        ...
        self.assertEqual(
            addr_dict["public_key_hash"], "b3fde27ce24e88b22a8340c903deaf91fade4d5a"
        )
    ...
    def test_key_address_p2sh_p2wpkh(self):
        ...
        self.assertEqual(
            addr.redeemscript,
            b"\x00\x14\x1e\xf4h\x07'\x1bM4RJ\xb9b\x11\xb9X\x81h\xdei:",
        )
    ...
    def test_keys_address_conversion(self):
        self.assertEqual(
            addr_convert(
                "1GMDUKLom6bJuY37RuFNc6PHv1rv2Hziuo", prefix="bc", to_encoding="bech32"
            ),
            "bc1q4pwfmstmw8q80nxtxud2h42lev9xzcjqwqyq7t",
        )
        self.assertEqual(
            addr_convert("1GMDUKLom6bJuY37RuFNc6PHv1rv2Hziuo", prefix=b"\x05"),
        ...
        self.assertEqual(
            ...
            "1GMDUKLom6bJuY37RuFNc6PHv1rv2Hziuo",
        )
        self.assertEqual(
            addr_convert("bc1q4pwfmstmw8q80nxtxud2h42lev9xzcjqwqyq7t", prefix="tb"),
    ...
    def test_keys_address_import_ambigue_bech32(self):
        ...
        self.assertEqual(
            deserialize_address(addr.address, encoding="bech32")["encoding"], "bech32"
        )

    def test_keys_hdkey_segwit(self):
        k1 = HDKey(
            "L1TZxZ9RgwFKiGPm6P7J9REQFKG9ymwLSsTwQSwxzLyDJs3CcRkF",
            witness_type="segwit",
        )
        self.assertEqual(k1.address(), "bc1qmk9myu4zf590ae2mfq3m63rlfhd5scatl4ckmw")
        self.assertEqual(
            k1.address_obj.data,
            "024429dd84f7c12c83f1920d60c7edb60c61d03fa5b8a8b526f4608ae9af89d9f3",
        )
        ...
        self.assertEqual(
            k2.address(),
            "bc1qvj6c7n0hpl9t5r80zya4uukf0zens8ulxgwc0avnxsengtr5juss4pqeqy",
        )
    ...
    def test_keys_address_p2tr(self):
        ...
        self.assertEqual(
            Address(
                hashed_data=public_hash, script_type="p2tr", encoding="bech32"
            ).address,
            address,
        )
...
class TestKeysDash(unittest.TestCase):
    ...
    def test_format_hdkey_private_dash(self):
        key = (
            "xprv9s21ZrQH143K3D4pKs8hj46ixU3T2vPsdmfMsoYjytd15C84SoRRkXebFFb3o4j6R5srg7btramafwcfdiibf2CWqMJLEX6jL2"
            "YUrLR7VfS"
        )
        self.assertEqual("hdkey_private", get_key_format(key)["format"])
    ...
    def test_dash_private_key(self):
        KC_DASH = Key(
            "000ece5e695793773007ac225a21fd570aa10f64d4da7ba29e6eabb0e34aae6b",
            network="dash_testnet",
        )
        self.assertEqual(
            KC_DASH.wif(), "cMapAmsnHr2UZ2ZCjZZfRru8dS9PLjYjTVjbnrR7suqducfQNYnX"
        )
        self.assertEqual(KC_DASH.address(), "ya3XLrAqfHFTFEZvDno9kv3MHREzHQzQMq")
        self.assertEqual(
            KC_DASH.public_hex,
            "02d092ed110b2d127c160ef1d72dc158fa96a3d32b41b9680ea6ef35e194bbc83e",
        )

    def test_hdkey_bip44_account_dash(self):
        pk = (
            "xprv9s21ZrQH143K3cq8ueA8GV9uv7cHqkyQGBQu8YZkAU2EXG5oSKVFeQnYK25zhHEEqqjfyTFEcV5enh6vh4tFA3FvdGuWAqPqvY"
            "ECNLB78mV"
        )
    ...
    def test_hdkey_dash(self):
        k = HDKey(
            "xprv9s21ZrQH143K4EGnYMHVxNp8JgqXCyywC3CGTrSzSudH3iRgC1gPTYgce4xamXMnyDAX8Qv8tvuW1LEgkZSrXiC25LqTJN"
            "8RpCKS5ixcQWD",
            network="dash",
        )
        self.assertEqual(
            "XkQ9Vudjgq62pvuG9K7pknVbiViZzZjWkJ", k.child_public(0).address()
        )
        self.assertEqual(
            "XtqfKEcdtn1QioGRie41uP79gGC6yPzmnz", k.child_public(100).address()
        )
        self.assertEqual(
            "XEYoxQJvhuXCXMpUFjf9knkJrFeE3mYp9mbFXG6mR3EK2Vvzi8vA",
            k.child_private(6).wif_key(),
        )
        self.assertEqual(
            "xprv9wZJLyzHEFzD3w3uazhGhbytbsVbrHQ5Spc7qkuwsPqUQo2VTxhpyoYRGD7o1T4AKZkfjGrWHtHrS4GUkBxzUH"
            "ozuqu8c2n3d7sjbmyPdFC",
            str(k.subkey_for_path("3H/1").wif(is_private=True)),
        )
...
class TestKeysSignatures(unittest.TestCase):
    def test_signatures(self):
        sig_tests = [
            ...
            (
                ...
                "b2da575054fb5daba0efde613b0b8e37159b8110e4be50f73cbe6479f6038f5b",
                1002,
            ...
            (
                ...
                "fd4b7465dadd8e8fe2dd8b8fa505c619e2959d5a6438daf7716a47e4617524a2",
                1234567890,
            ...
            (
                ...
                HDKey(
                    "xprv9s21ZrQH143K2YEun3sBzwSaFLn6bnBa6nkodJrDfZSty6L7Ba9JR5tMdhc7viB9dPu6LpQ9UqrsDsrJ8GNLQHf4SKA"
                    "zGrXL6Pp5kjojqzi",
                    network="bitcoin",
                ),
        ...
        self.assertEqual(
            sig_method1.as_der_encoded(include_hash_type=False).hex(), sig_tests[0][4]
        )
        count = 0
        for case in sig_tests:
            ...
            self.assertEqual(
                sig.hex(),
                case[3],
                msg="Error in #%d: %s != %s" % (count, sig.hex(), case[3]),
            )
    ...
    def test_signatures_rfc6979(self):
        ...
        test_vectors = [
            ...
            (
                ...
                "not learning anywhere near enough",
                None,
    ...
    def test_signatures_from_r_and_s(self):
        ...
        expected_der = (
            "30450220657912a72d3ac8169fe8eaecd5ab401c94fc9981717e3e6dd4971889f785790c022100ed3bf345"
            "6eb76677fd899c8ccd1cc6d1ebc631b94c42f7c4578f28590d651c6e01"
        )
        expected_sig_bytes = (
            b"ey\x12\xa7-:\xc8\x16\x9f\xe8\xea\xec\xd5\xab@\x1c\x94\xfc\x99\x81q~>m\xd4\x97\x18"
            b"\x89\xf7\x85y\x0c\xed;\xf3En\xb7fw\xfd\x89\x9c\x8c\xcd\x1c\xc6\xd1\xeb\xc61\xb9LB"
            b"\xf7\xc4W\x8f(Y\re\x1cn"
        )
        expected_sig_hex = (
            "657912a72d3ac8169fe8eaecd5ab401c94fc9981717e3e6dd4971889f785790ced3bf3456eb76677fd899"
    ...
    def test_signatures_rs_out_of_curve(self):
        ...
        self.assertRaisesRegexp(
            BKeyError,
            "r is not a positive integer smaller than the curve order",
            Signature,
            outofcurveint,
            10,
        )
        ...
        self.assertRaisesRegexp(
            BKeyError,
            "s is not a positive integer smaller than the curve order",
            Signature,
            11,
            outofcurveint,
        )
    ...
    def test_signatures_dunder(self):
        sig1 = Signature.parse_hex(
            "3045022100c949a465a057f3ca7d20e80511e93d0be21e3efbeb8720ca3e0adfbce6883d0a022070b"
            "2c6bee101a4ffcb854bae34dbd1f35c31140a46559148a1fa883eedede03401"
        )
        sig2 = Signature.parse_hex(
            "3045022100b5ce13dc408c65208cf475b44b2012845d4d3fb7a2cacfa35f6b5143761f976f02207d8"
        ...
        self.assertEqual(
            sig1 + sig2,
            b"0E\x02!\x00\xc9I\xa4e\xa0W\xf3\xca} \xe8\x05\x11\xe9=\x0b\xe2\x1e>"
        ...
        self.assertEqual(
            str(sig1),
            "3045022100c949a465a057f3ca7d20e80511e93d0be21e3efbeb8720ca3e0adfbce6883d0a0220"
...
class TestKeysTaproot(unittest.TestCase):
    def test_keys_taproot_addresses(self):
        ...
        for address in addresses:
            addr_dict = deserialize_address(address)
            address_encode = pubkeyhash_to_addr_bech32(
                addr_dict["public_key_hash_bytes"],
                addr_dict["prefix"],
                addr_dict["witver"],
            )
        ...
        address_fantasy = deserialize_address(
            "lc108denc9m4sqe9hluasrvxkkdqgkydrk5ctxre5nkk4qwdvefn0sdsggm3lr"
        )
        self.assertEqual(address_fantasy["witver"], 15)
        self.assertEqual(
            address_fantasy["public_key_hash"],
        ...
        p2tr_address = pubkeyhash_to_addr_bech32(
            pubkeyhash, "bc", checksum_xor=BECH32M_CONST
        )
