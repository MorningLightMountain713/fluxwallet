...
DATABASEFILE_UNITTESTS = os.path.join(str(FW_DATABASE_DIR), 'fluxwallet.unittest.sqlite')
DATABASEFILE_UNITTESTS_2 = os.path.join(str(FW_DATABASE_DIR), 'fluxwallet.unittest2.sqlite')
...
class TestWalletCreate(TestWalletMixin, unittest.TestCase):
    ...
    def test_delete_wallet_no_private(self):
        ...
        Wallet.create('delete_watch_only_wallet', keys=key, network='fluxwallet_test', db_uri=self.DATABASE_URI)
    ...
    def test_wallet_delete_not_empty(self):
        w = Wallet.create('unempty_wallet_test', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        w.utxos_update()
    ...
    def test_wallet_as_dict_json(self):
        wallet = Wallet.create("test_wallet_as_dict_json", db_uri=self.DATABASE_URI, network='fluxwallet_test')
...
class TestWalletKeys(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_create_uncompressed_masterkey(self):
        wlt = wallet_create_or_open('uncompressed_test', keys='68vBWcBndYGLpd4KmeNTk1gS1A71zyDX6uVQKCxq6umYKyYUav5',
                                    network='fluxwallet_test', db_uri=self.DATABASE_URI)
        wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_single_key(self):
        wlt = wallet_create_or_open('single_key', scheme='single', network='fluxwallet_test',
                                    db_uri=self.DATABASE_URI)
        wlt.utxos_update()
...
class TestWalletMultiCurrency(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_multiple_networks_import_error(self):
        pk_dashtest = 'YXsfembHRwpatrAVUGY8MBUuKwhUDf9EEWeZwGoEfE5appg5rLjSfZ1GwoaNB5DgUZ2aVuU1ezmg7zDefubuWkZ17et5o' \
                      'KoMgKnjvAZ6a4Yn2QZg'
        error_str = "Network fluxwallet_test not available in this wallet, please create an account for this network " \
                    "first."
...
class TestWalletMultiNetworksMultiAccount(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_multi_networks_send_transaction(self):
        ...
        self.assertEqual(wallet.balance(network='fluxwallet_test', account_id=1), 600000000)
        ...
        t = wallet.send_to('21EsLrvFQdYWXoJjGX8LSEGWHFJDzSs2F35', 10000000, account_id=1,
                           network='fluxwallet_test', fee=1000, offline=False)
        self.assertIsNone(t.error)
        self.assertTrue(t.verified)
        self.assertEqual(wallet.balance(network='fluxwallet_test', account_id=1), 589999000)
        self.assertEqual(len(wallet.transactions(account_id=0, network='fluxwallet_test')), 0)
        self.assertEqual(len(wallet.transactions(account_id=1, network='fluxwallet_test')), 7)
        del wallet
...
class TestWalletMultisig(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_multisig_fluxwallet_testnet_transaction_send(self):
        ...
        wl = Wallet.create('multisig_test_simple', key_list, sigs_required=2, network='fluxwallet_test',
    ...
    def test_wallet_multisig_2of2_different_database(self):
        ...
        msw1 = Wallet.create('msw1', [keys[0], keys[1].public_master(multisig=True)], network='fluxwallet_test',
                             sort_keys=False, sigs_required=2, db_uri=self.DATABASE_URI)
        msw2 = Wallet.create('msw2', [keys[0].public_master(multisig=True), keys[1]], network='fluxwallet_test',
    ...
    def test_wallet_multisig_2of2_with_single_key(self):
        self.db_remove()
        keys = [HDKey(network='fluxwallet_test'), HDKey(network='fluxwallet_test', key_type='single')]
        key_list = [keys[0], keys[1].public()]

        wl = Wallet.create('multisig_expk2', key_list, sigs_required=2, network='fluxwallet_test',
        ...
        t = wl.transaction_create([(HDKey(network='fluxwallet_test').address(), 6400000)], min_confirms=0)
        t.sign(keys[1])
        t.send()
        self.assertIsNone(t.error)
    ...
    def test_wallet_multisig_info(self):
        w = Wallet.create('test_wallet_multisig_info', keys=[HDKey(network='fluxwallet_test'),
                                                             HDKey(network='fluxwallet_test')],
                          network='fluxwallet_test', cosigner_id=0, db_uri=self.DATABASE_URI)
        w.utxos_update()
        w.info(detail=6)
...
class TestWalletKeyImport(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_import_private_for_known_public_p2sh_segwit(self):
        ...
        w = Wallet.create('segwit-p2sh-p2wsh-import',
                          ...
                          witness_type='p2sh-segwit', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        w.get_key()
        w.utxos_update()
...
class TestWalletTransactions(TestWalletMixin, unittest.TestCase, CustomAssertions):
    ...
    def test_wallet_two_utxos_one_key(self):
        wlt = Wallet.create('double-utxo-test', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        key = wlt.new_key()
        wlt.utxos_update()
        utxos = wlt.utxos()
    ...
    def test_wallet_balance_update(self):
        wlt = Wallet.create('test-balance-update', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_balance_update_multi_network(self):
        k = "tpubDCutwJADa3iSbFtB2LgnaaqJgZ8FPXRRzcrMq7Tms41QNnTV291rpkps9vRwyss9zgDc7hS5V1aM1by8nFip5VjpGpz1oP54peKt" \
            "hJzfabX"
        wlt = Wallet.create("test_wallet_balance_update_multi_network", network='fluxwallet_test',
                            db_uri=self.DATABASE_URI)
        wlt.new_key()
    ...
    def test_wallet_add_dust_to_fee(self):
        ...
        wlt = Wallet.create('bcltestwlt', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transactions_send_update_utxos(self):
        ...
        wlt = Wallet.create('bcltestwlt2', network='fluxwallet_test', db_uri=self.DATABASE_URI)
    ...
    def test_wallet_transaction_import(self):
        wlt = Wallet.create('bcltestwlt3', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_raw(self):
        wlt = Wallet.create('bcltestwlt4', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_raw_locktime(self):
        wlt = Wallet.create('bcltestwlt4b', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_raw_segwit_fee(self):
        ...
        pk = 'YXscyqNJ5YK411nwB2peYdMeJPmkJmMJCfNdo9JuWkEKLZhVSoUjbRRinVqqtBN2GNC2A6L1Taz1e3LWApHkC84GgTp3vr7neD' \
             'ZTxXnvGkUwVz4c'
        wlt = Wallet.create('bcltestwlt-size', keys=pk, network='fluxwallet_test', witness_type='segwit',
                            db_uri=self.DATABASE_URI)
        wlt.utxos_update()
    ...
    def test_wallet_transaction_load_segwit_size(self):
        pk = 'YXscyqNJ5YK411nwB2peYdMeJPmkJmMJCfNdo9JuWkEKLZhVSoUjbRRinVqqtBN2GNC2A6L1Taz1e3LWApHkC84GgTp3vr7neD' \
             'ZTxXnvGkUwVz4c'
        wlt = Wallet.create('bcltestwlt2-size', keys=pk, network='fluxwallet_test', witness_type='segwit',
                            db_uri=self.DATABASE_URI)
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_dict(self):
        wlt = Wallet.create('bcltestwlt5', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_fee_limits(self):
        wlt = Wallet.create('bcltestwlt6', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_fee_zero_problem(self):
        wlt = Wallet.create(name='bcltestwlt7', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        nk = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_sign_with_hex(self):
        ...
        pmk = k.public_master()
        w = Wallet.create('wallet_tx_tests', keys=pmk, network='fluxwallet_test', db_uri=self.DATABASE_URI)
        w.utxos_update()
    ...
    def test_wallet_transaction_sign_with_wif(self):
        ...
        w = wallet_create_or_open('test_wallet_transaction_sign_with_wif',
                                  ...
                                  witness_type='segwit', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        w.get_key()
        w.utxos_update()
    ...
    def test_wallet_transaction_restore_saved_tx(self):
        w = wallet_create_or_open('test_wallet_transaction_restore', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        wk = w.get_key()
    ...
    def test_wallet_transaction_send_keyid(self):
        w = Wallet.create('wallet_send_key_id', witness_type='segwit', network='fluxwallet_test',
                          db_uri=self.DATABASE_URI)
    ...
    def test_wallet_transactions_sign_with_mnenomic(self):
        ...
        w = wallet_create_or_open('wlt-sign-with-mnemonic', keys=prv_key.public(), network='fluxwallet_test',
                                  scheme='single', db_uri=self.DATABASE_URI)
        w.new_key()
        w.utxos_update()
    ...
    def test_wallet_select_inputs(self):
        wlt = Wallet.create('bclwltsltinputs', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_save_and_load(self):
        w = wallet_create_or_open('wallet_transaction_save_and_load', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        w.utxos_update()
    ...
    def test_wallet_transaction_save_and_load_filename(self):
        w = wallet_create_or_open('wallet_transaction_save_and_load_filename', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        w.utxos_update()
    ...
    def test_wallet_avoid_forced_address_reuse(self):
        w = wallet_create_or_open('wallet_avoid_forced_address_reuse_test', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        k1 = w.get_key()
        w.utxos_update()
        k2 = w.new_key()
    ...
    def test_wallet_transactions_delete(self):
        w = wallet_create_or_open('wallet_transactions_delete', network='fluxwallet_test', db_uri=self.DATABASE_URI)
        w.utxos_update()
        w.transactions()[0].delete()
        self.assertEqual(len(w.transactions()), 1)

    def test_wallet_create_import_key(self):
        w = wallet_create_or_open("test_wallet_create_import_key_private", network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)

        w.utxos_update()
        wk = w.public_master()
        w2 = wallet_create_or_open('test_wallet_create_import_key_public', network='fluxwallet_test', keys=wk,
                                   db_uri=self.DATABASE_URI)
        w2.utxos_update()
    ...
    def test_wallet_transactions_pagination(self):
        w = wallet_create_or_open("bclt_wallet_pagination_test", network='fluxwallet_test', db_uri=self.DATABASE_URI)
...
class TestWalletDash(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_merge_transactions(self):
        w = wallet_create_or_open('wallet_merge_transactions_tests', network='fluxwallet_test',
                                  db_uri=self.DATABASE_URI)
        w.utxos_update()
        u = w.utxos()
...
class TestWalletSegwit(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_segwit_p2wpkh_send(self):
        w = Wallet.create('segwit_p2wpkh_send', witness_type='segwit', network='fluxwallet_test',
                          db_uri=self.DATABASE_URI)
        w.get_key()
        w.utxos_update()
    ...
    def test_wallet_segwit_p2wsh_send(self):
        w = Wallet.create('segwit_p2wsh_send', witness_type='segwit', network='fluxwallet_test',
                          keys=[HDKey(network='fluxwallet_test'), HDKey(network='fluxwallet_test')], sigs_required=2,
                          cosigner_id=0, db_uri=self.DATABASE_URI)
        k = w.get_key()
        w.utxos_update()
    ...
    def test_wallet_segwit_p2sh_p2wpkh_send(self):
        w = Wallet.create('segwit_p2sh_p2wpkh_send', witness_type='p2sh-segwit', network='fluxwallet_test',
                          db_uri=self.DATABASE_URI)
        w.get_key()
        w.utxos_update()
        t = w.send_to('blt1q7ywlg3lsyntsmp74jh65pnkntk3csagdwpz78k', 10000, offline=False)
    ...
    def test_wallet_segwit_p2sh_p2wsh_send(self):
        w = Wallet.create('segwit_p2sh_p2wsh_send', witness_type='p2sh-segwit', network='fluxwallet_test',
                          keys=[HDKey(network='fluxwallet_test'), HDKey(network='fluxwallet_test'),
                                HDKey(network='fluxwallet_test')], sigs_required=2, cosigner_id=0,
                          db_uri=self.DATABASE_URI)
        w.get_key()
        w.utxos_update()
        t = w.send_to('blt1q7ywlg3lsyntsmp74jh65pnkntk3csagdwpz78k', 10000, offline=False)
    ...
    def test_wallet_segwit_uncompressed_error(self):
        ...
        self.assertRaisesRegexp(BKeyError, 'Uncompressed keys are non-standard', wallet_create_or_open,
                                'segwit_uncompressed_error', k, witness_type='segwit', network='fluxwallet_test',
                                db_uri=self.DATABASE_URI)
    ...
    def test_wallet_segwit_multisig_multiple_inputs(self):
        ...
        w = wallet_create_or_open('test_wallet_segwit_multisig_multiple_inputs',
                                  ...
                                  witness_type='segwit', network='fluxwallet_test', db_uri=self.DATABASE_URI)
