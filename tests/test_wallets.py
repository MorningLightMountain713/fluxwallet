...
from fluxwallet.encoding import USE_FASTECDSA
from fluxwallet.keys import BKeyError, HDKey
...
db_uris = (
    (
        "sqlite",
        "sqlite:///" + DATABASEFILE_UNITTESTS,
        "sqlite:///" + DATABASEFILE_UNITTESTS_2,
    ),
)
...
if UNITTESTS_FULL_DATABASE_TEST:
    db_uris += (
        (
            "mysql",
            "mysql://root:root@localhost:3306/" + DATABASE_NAME,
...
params = (("SCHEMA", "DATABASE_URI", "DATABASE_URI_2"), (db_uris))
    ...
    def db_remove(cls):
        ...
        if cls.SCHEMA == "sqlite":
            ...
            for db in [DATABASE_NAME, DATABASE_NAME_2]:
                ...
                con = psycopg2.connect(
                    user="postgres", host="localhost", password="postgres", database=db
                )
                con.set_isolation_level(ISOLATION_LEVEL_AUTOCOMMIT)
                cur = con.cursor()
                try:
            ...
            for db in [DATABASE_NAME, DATABASE_NAME_2]:
                ...
                con = mysql.connector.connect(
                    user="root", host="localhost", database=db, autocommit=True
                )
...
class TestWalletCreate(TestWalletMixin, unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.db_remove()
        cls.wallet = Wallet.create(name="test_wallet_create", db_uri=cls.DATABASE_URI)
    ...
    def test_delete_wallet(self):
        Wallet.create(name="wallet_to_remove", db_uri=self.DATABASE_URI)
    ...
    def test_delete_wallet_no_private(self):
        ...
        Wallet.create(
            "delete_watch_only_wallet",
            keys=key,
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        self.assertEqual(
            wallet_delete("delete_watch_only_wallet", db_uri=self.DATABASE_URI), 1
        )
    ...
    def test_wallet_empty(self):
        ...
        w2 = Wallet.create(
            "empty_wallet_test2", keys=master_key, db_uri=self.DATABASE_URI
        )
        w3 = Wallet.create(
            "empty_wallet_test3", keys=master_key, db_uri=self.DATABASE_URI
        )
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Wallet 'unknown_wallet_2' not found",
            wallet_empty,
            "unknown_wallet_2",
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_delete_not_empty(self):
        w = Wallet.create(
            "unempty_wallet_test", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        w.utxos_update()
        self.assertRaisesRegexp(
            WalletError,
            "still has unspent outputs. Use 'force=True' to delete this wallet",
            wallet_delete,
            "unempty_wallet_test",
            db_uri=self.DATABASE_URI,
        )
        self.assertTrue(
            wallet_delete("unempty_wallet_test", db_uri=self.DATABASE_URI, force=True)
        )

    def test_delete_wallet_exception(self):
        self.assertRaisesRegexp(
            WalletError, "", wallet_delete, "unknown_wallet", db_uri=self.DATABASE_URI
        )

    def test_wallet_unknown_error(self):
        self.assertRaisesRegexp(
            WalletError,
            "Wallet 'test_wallet_create_errors10' not found",
            Wallet,
            "test_wallet_create_errors10",
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_create_errors(self):
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Wallet with name 'test_wallet_create_errors' already exists",
            Wallet.create,
            "test_wallet_create_errors",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            ...
            Wallet.create,
            "test_wallet_create_errors2",
            scheme="raar",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Wallet name '123' invalid, please include letter characters",
            Wallet.create,
            "123",
            db_uri=self.DATABASE_URI,
        )
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Witness type unknown not supported at the moment",
            Wallet.create,
            "",
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Multisig wallets should use bip32 scheme not single",
            Wallet.create,
            "test_wallet_create_errors_multisig",
            keys=[HDKey(), HDKey()],
            scheme="single",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Password protected multisig wallets not supported",
            Wallet.create,
            "test_wallet_create_errors_multisig2",
            keys=[HDKey(), HDKey()],
            password="geheim",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            ...
            Wallet.create,
            "test_wallet_create_errors_multisig3",
            keys=[HDKey(), HDKey()],
            sigs_required=3,
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            ...
            Wallet.create,
            "test_wallet_create_errors_multisig4",
            keys=[HDKey(), HDKey(network="dash")],
            db_uri=self.DATABASE_URI,
        )
        passphrase = (
            "usual olympic ride small mix follow trend baby stereo sweet lucky lend"
        )
        self.assertRaisesRegexp(
            WalletError,
            "Please specify network when using passphrase to create a key",
            Wallet.create,
            "test_wallet_create_errors3",
            keys=passphrase,
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Invalid key or address: zwqrC7h9pRj7SBhLRDG4FnkNBRQgene3y3",
            Wallet.create,
            "test_wallet_create_errors4",
            keys="zwqrC7h9pRj7SBhLRDG4FnkNBRQgene3y3",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Segwit is not supported for Dash wallets",
            Wallet.create,
            "test_wallet_create_errors6",
            keys=HDKey(network="dash"),
            witness_type="segwit",
            db_uri=self.DATABASE_URI,
        )
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Depth of provided public master key 7 does not correspond with key path",
            Wallet.create,
            "test_wallet_create_errors7",
            keys=k,
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_rename_duplicate(self):
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Wallet with name 'test_wallet_rename_duplicate1' already exists",
            test_func,
        )

    def test_wallet_as_dict_json(self):
        wallet = Wallet.create(
            "test_wallet_as_dict_json",
            db_uri=self.DATABASE_URI,
            network="fluxwallet_test",
        )
    ...
    def test_wallet_create_bip38(self):
        ...
        passphrase = (
            "region kite swamp float card flag chalk click gadget share wage clever"
        )
...
class TestWalletImport(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_import(self):
        keystr = (
            "tprv8ZgxMBicQKsPeWn8NtYVK5Hagad84UEPEs85EciCzf8xYWocuJovxsoNoxZAgfSrCp2xa6DdhDrzYVE8UXF75r2dKePy"
            "A7irEvBoe4aAn52"
        )
        ...
        self.assertEqual(
            wallet_import.main_key.address, "n3UKaXBRDhTVpkvgRH7eARZFsYE989bHjw"
        )
        self.assertEqual(wallet_import.main_key.path, "m")

    def test_wallet_import_account(self):
        accountkey = (
            "tprv8h4wEmfC2aSckSCYa68t8MhL7F8p9xAy322B5d6ipzY5ZWGGwksJMoajMCqd73cP4EVRygPQubgJPu9duBzPn3QV"
            "8Y7KbKUnaMzx9nnsSvh"
        )
        wallet_import = Wallet.create(
            db_uri=self.DATABASE_URI,
        ...
        self.assertEqual(
            wallet_import.main_key.address, "mowRx2TNXTgRSUmepjqhx5C1TTigmHLGRh"
        )
        self.assertEqual(wallet_import.main_key.path, "M")
    ...
    def test_wallet_import_account_new_keys(self):
        ...
        self.assertEqual(wallet_import.main_key.wif, accountkey)
        self.assertEqual(newkey.address, "mxdLD8SAGS9fe2EeCXALDHcdTTbppMHp8N")
        self.assertEqual(newkey.path, "M/0/1")
        self.assertEqual(newkey.account_id, 99)
        self.assertEqual(newkey_change.address, "mkzpsGwaUU7rYzrDZZVXFne7dXEeo6Zpw2")
    ...
    def test_wallet_import_public_wallet(self):
        pubkey = (
            "tpubDDkyPBhSAx8DFYxx5aLjvKH6B6Eq2eDK1YN76x1WeijE8eVUswpibGbv8zJjD6yLDHzVcqWzSp2fWVFhEW9XnBssFqMwt"
            "9SrsVeBeqfBbR3"
        )
        ...
        self.assertEqual(newkey.address, "myitDjbzYpUTShv9CyXRJKXtM4uRgSqa3A")
    ...
    def test_wallet_import_litecoin(self):
        ...
        self.assertEqual(wallet_import.main_key.wif, accountkey)
        self.assertEqual(newkey.address, "LZj8MnR6tRgLNKUBSfd2pD2czA4F9G5oGk")
        self.assertEqual(newkey.path, "m/44'/2'/0'/0/1")

    def test_wallet_import_key_network_error(self):
        w = Wallet.create(name="Wallet Error", db_uri=self.DATABASE_URI)
        self.assertRaisesRegexp(
            WalletError,
    ...
    def test_wallet_import_hdwif(self):
        ...
        self.assertEqual(
            w.get_key().address, "bc1qruvyu8f2tg06zhysytdsc3qlngnpfzn0juwssx"
        )
        self.assertEqual(
            w.get_key_change().address, "bc1qg6h45txt82x87uvv3ndm82xsf3wjknq8j7sufh"
        )
        ...
        self.assertEqual(
            w.get_key_change().address, "33Un3fDSdT2hsuqyuHiCci1GyUbiyZEWHW"
        )
        ...
        w = Wallet.create(
            "wif_import_p2wsh", [wif1, wif2], cosigner_id=0, db_uri=self.DATABASE_URI
        )
        self.assertEqual(
            w.get_key().address,
            "bc1qhk5pd2fh0uea8z2wkceuf8eqlxsnkxm0hlt6qvfv346evqnfyumqucpqew",
        )
        self.assertEqual(
            w.get_key_change().address,
            "bc1qyn73qh408ry38twxnj4aqzuyv7j6euwhwt2qtzayg673vtw2a4rsn7jlek",
        )
        ...
        self.assertEqual(
            w.get_key_change().address, "3PFD1qkgbaeeDnX38Smerb5vAPBkDVkhcm"
        )

    def test_wallet_import_master_key(self):
        k = HDKey()
        w = Wallet.create(
            "test_wallet_import_master_key",
            keys=k.public_master(),
            db_uri=self.DATABASE_URI,
        )
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Network of Wallet class, main account key and the imported private "
            "key must use the same network",
            w.import_master_key,
            HDKey(network="litecoin"),
        )
        self.assertRaisesRegexp(
            WalletError,
        ...
        k2 = HDKey()
        w2 = Wallet.create(
            "test_wallet_import_master_key2",
            keys=k2.subkey_for_path("m/32'"),
            scheme="single",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Main key is already a private key, cannot import key",
            w2.import_master_key,
            k2,
        )
        w2.main_key = None
        self.assertRaisesRegexp(
            WalletError,
            "Main wallet key is not an WalletKey instance",
            w2.import_master_key,
            k2,
        )
        ...
        k3 = HDKey()
        w3 = Wallet.create(
            "test_wallet_import_master_key3",
            keys=k3.subkey_for_path("m/32'").public(),
            scheme="single",
            db_uri=self.DATABASE_URI,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Current main key is not a valid BIP32 public master key",
            w3.import_master_key,
            k3,
        )
...
class TestWalletExport(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_export_hdwifs(self):
        ...
        wif = (
            "zpub6s7HTSrGmNUWSgfbDMhYbXVuxA14yNnycS25v6ogicEauzUrRUkuCLQUWbJXP1NyXNqGmwpU6hZw7vr22a4yspwH8XQFjjwRmx"
            "CKkXdDAXN"
        )
        self.assertEqual(
            w.account(0).key().wif_public(witness_type=w.witness_type), wif
        )
        ...
        wif = (
            "ypub6YMgBd4GfQjtxUf8ExorFUQEpBfUYTDz7E1tvfNgDqZeDEUuNNVXSNfsebis2cyeqWYXx6yaBBEQV7sJW3NGoXw5wsp9kkEsB2"
            "DqiVquYLE"
        )
        self.assertEqual(w.wif(is_private=False), wif)
        ...
        p3 = (
            "valid circle lounge pipe alone stool system off until physical juice opera"
        )
        wifs = [
...
class TestWalletKeys(TestWalletMixin, unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.db_remove()
        cls.private_wif = (
            "xprv9s21ZrQH143K24Mfq5zL5MhWK9hUhhGbd45hLXo2Pq2oqzMMo63oStZzF9ySUHZw5qJkk5LCALAhXS"
            "XoCmCSnStRvgwLBtcbGsg1PeKT2en"
        )
        cls.wallet = Wallet.create(
            keys=cls.private_wif, name="test_wallet_keys", db_uri=cls.DATABASE_URI
        )
        cls.wallet.new_key()
    ...
    def test_wallet_addresslist(self):
        expected_addresslist = [
            "1B8gTuj778tkrQV1e8qjcesoZt9Cif3VEp",
            "1LS8zYrkgGpvJdtMmUdU1iU4TUMQh6jjF1",
            "1K7S5am1hLfugEFWR9ENfEBpUrMbFhqtoh",
            "1EByrVS1sc6TDihJRRRtMAnKTaAVSZAgtQ",
            "1KyLsZS2JwWdfvDZ5g8vhbanqjbNwKUseK",
            "1J6jppU5mWf4ausGfHMumrKrztpDKq2MrD",
            "12ypWFxJSKWknmvxdSeStWCyVDBi8YyXpn",
            "1A7wRpnstUiA33rxW1i33b5qqaTsS4YSNQ",
    ...
    def test_wallet_keys_method_masterkey(self):
        self.assertEqual(
            self.wallet.keys(name="test_wallet_keys", depth=0)[0].wif, self.private_wif
        )

    def test_wallet_keys_method_account(self):
        account_wif = (
            "xprv9z87HKmxfjVRRyEt7zBWCctmJvqcowfWwKUeJnLjNyykvq5sDGm1yo5qTWWAj1gXsRd2b8GayjujPz1arbKsS3tnwQ"
            "Fz8nMip3pFBYjPT1b"
        )
    ...
    def test_wallet_keys_method_keys_addresses(self):
        ...
        self.assertListEqual(
            [k.wif for k in self.wallet.keys_addresses()], address_wifs
        )

    def test_wallet_keys_method_keys_payment(self):
        self.assertEqual(
            self.wallet.keys_address_payment()[0].address,
            "1J6jppU5mWf4ausGfHMumrKrztpDKq2MrD",
        )

    def test_wallet_keys_method_keys_change(self):
        self.assertEqual(
            self.wallet.keys_address_change()[0].address,
            "13uQKuiWwWp15BsEijnpKZSuTuHVTpZMvP",
        )
    ...
    def test_wallet_keys_single_key(self):
        self.db_remove()
        wk = (
            "xprv9s21ZrQH143K3tCgu8uhkA2fw9F9opbvoNNzh5wcuEvNHbCU6Kg3c6dam2a6cw4UYeDxAsgBorAqXp2nsoYS84DqYMwkzxZ15"
            "ujRHzmBMxE"
        )
        w = Wallet.create(
            "test_wallet_keys_single_key", wk, scheme="single", db_uri=self.DATABASE_URI
        )
    ...
    def test_wallet_create_uncompressed_masterkey(self):
        ...
        wlt.utxos_update()
        self.assertIsNone(
            wlt.sweep("216xtQvbcG4o7Yz33n7VCGyaQhiytuvoqJY", offline=False).error
        )
    ...
    def test_wallet_single_key(self):
        ...
        wlt.utxos_update()
        transaction = wlt.transaction_create(
            [("21DQCyZTNRoAccG1TWz9YaffDUKzZf6JWii", 90000000)]
        )
        transaction.sign()
    ...
    def test_wallet_single_key_main_key(self):
        w = Wallet.create(
            "multisig_with_single_key",
            [HDKey().public_master_multisig(), HDKey(key_type="single")],
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
        w.new_key()
    ...
    def test_wallet_private_parts(self):
        ...
        w.utxo_add(
            w.main_key.address,
            200000,
            "46fcfdbdc3573756916a0ced8bbc5418063abccd2c272f17bf266f77549b62d5",
            0,
            1,
        )
        ...
        wif2 = (
            "xprv9s21ZrQH143K3mEc645dz1wEo4F1Sy8mNKdXZxFWniS7ZNqSgo2CS7chiDxqvVHJbQeR7RsMPaUaeTL6nwD9ChnwJw4LHz"
            "Ni2xTDTQ8t1Hn"
        )
        private_hex2 = (
            "e68f716a02929e29c5b9b3d8a1a1b5424dc63ea03eb9b9990cf0ffaf396789fc"
        )
        secret2 = 104285397059777051994770481926675935425657204513917549168323819901909731871228
        k2 = HDKey(wif2)

        wms = wallet_create_or_open(
            "wlttest_ms", [k, k2], cosigner_id=0, db_uri=self.DATABASE_URI
        )
        w_json = wms.as_json()
    ...
    def test_wallet_key_create_from_key(self):
        ...
        w1 = Wallet.create(
            "network_mixup_test_wallet", network="litecoin", db_uri=self.DATABASE_URI
        )
        ...
        w2 = Wallet.create(
            "network_mixup_test_wallet2", network="litecoin", db_uri=self.DATABASE_URI
        )
    ...
    def test_wallet_key_exceptions(self):
        ...
        self.assertRaisesRegexp(
            WalletError, "Key with id 1000000 not found", WalletKey, 1000000, w._session
        )
        self.assertRaisesRegexp(
            BKeyError,
            "Specified key \['litecoin', 'litecoin_legacy'\] is from different "
    ...
    def test_wallet_key_public_leaks(self):
        w = wallet_create_or_open(
            "wallet_private", network="testnet", db_uri=self.DATABASE_URI
        )
        wk = w.public_master()
        ...
        w2 = wallet_create_or_open(
            "wallet_public", network="testnet", keys=wk, db_uri=self.DATABASE_URI
        )
...
class TestWalletElectrum(TestWalletMixin, unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.db_remove()
        cls.pk = (
            "xprv9s21ZrQH143K2fuscnMTwUadsPqEbYdFQVJ1uWPawUYi7C485NHhCiotGy6Kz3Cz7ReVr65oXNwhREZ8ePrz8p7zy"
            "Hra82D1EGS7cQQmreK"
        )
        cls.wallet = Wallet.create(
            keys=cls.pk, name="test_wallet_electrum", db_uri=cls.DATABASE_URI
        )
    ...
    def test_wallet_electrum_p2pkh(self):
        phrase = (
            "smart donor clever resource stool denial wink under oak sand limb wagon"
        )
        wlt = Wallet.create(
            "wallet_electrum_p2pkh",
            phrase,
            network="bitcoin",
            witness_type="segwit",
            db_uri=self.DATABASE_URI,
        ...
        self.assertEqual(
            wlt.get_key().address, "bc1qjz5l6mej6ptqlggfvdlys8pfukwqp8xnu0mn5u"
        )
        self.assertEqual(
            wlt.get_key_change().address, "bc1qz4tr569wfs2fuekgcjtdlz0eufk7rfs8gnu5j9"
        )
    ...
    def test_wallet_electrum_p2sh_p2wsh(self):
        ...
        phrase2 = (
            "wink tornado honey delay nest sing series timber album region suit spawn"
        )
        wlt = Wallet.create(
            "wallet_electrum_p2sh_p2wsh",
            [phrase1, phrase2],
            network="bitcoin",
            cosigner_id=0,
            witness_type="p2sh-segwit",
...
class TestWalletMultiCurrency(TestWalletMixin, unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.db_remove()
        cls.pk = (
            "xprv9s21ZrQH143K4478MENLXpSXSvJSRYsjD2G3sY7s5sxAGubyujDmt9Qzfd1me5s1HokWGGKW9Uft8eB9dqryybAcFZ5JAs"
            "rg84jAVYwKJ8c"
        )
        cls.wallet = Wallet.create(
            keys=cls.pk,
    ...
    def test_wallet_multiple_networks_default_addresses(self):
        addresses_expected = [
            "XqTpf6NYrrckvsauJKfHFBzZaD9wRHjQtv",
            "Xamqfy4y21pXMUP8x8TeTPWCNzsKrhSfau",
            "XugknDhBtJFvfErjaobizCa8KAEDVU7bCJ",
            "Xj6tV9Jc3qJ2AszpNxvEq7KVQKUMcfmBqH",
            "XgkpZbqaRsRb2e2BC1QsWxTDEfW6JVpP6r",
        ]
        self.assertListEqual(
            self.wallet.addresslist(network="dash"), addresses_expected
        )

    def test_wallet_multiple_networks_import_key(self):
        pk_bitcoin = (
            "xprv9s21ZrQH143K3RBvuNbSwpAHxXuPNWMMPfpjuX6ciwo91HpYq6gDLjZuyrQCPpo4qBDXyvftN7MdX7SBVXeGgHs"
    ...
    def test_wallet_multiple_networks_import_error(self):
        ...
        error_str = (
            "Network fluxwallet_test not available in this wallet, please create an account for this network "
            "first."
        )
        self.assertRaisesRegexp(
            WalletError, error_str, self.wallet.import_key, pk_dashtest
        )
    ...
    def test_wallet_multiple_networks_value(self):
        ...
        wallet_delete_if_exists(
            "test_wallet_multiple_networks_value", force=True, db_uri=self.DATABASE_URI
        )
        w = wallet_create_or_open(
            "test_wallet_multiple_networks_value", keys=pk, db_uri=self.DATABASE_URI
        )
        w.new_account(network="bitcoin")
        w.new_account(network="fluxwallet_test")
        w.utxos_update(networks="testnet")
        self.assertEqual(len(w.utxos(network="testnet")), 1)
        w.utxos_update(networks="fluxwallet_test")
        self.assertEqual(len(w.utxos(network="fluxwallet_test")), 4)
        t = w.send_to(
            "blt1qctnl4yk3qepjy3uu36kved5ds6q9g8c6raan7l", "50 mTST", offline=False
        )
...
class TestWalletMultiNetworksMultiAccount(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_multi_networks_send_transaction(self):
        pk = (
            "tobacco defy swarm leaf flat pyramid velvet pen minor twist maximum extend"
        )
        wallet = Wallet.create(
            keys=pk,
            network="bitcoin",
        ...
        wallet.new_key()
        acc = wallet.new_account("BCL test home", network="fluxwallet_test")
        acc2 = wallet.new_account("BCL test office", network="fluxwallet_test")
        wallet.new_key(account_id=acc2.account_id, network="fluxwallet_test")
        wallet.new_key(account_id=acc.account_id, network="fluxwallet_test")
        wallet.utxos_update(networks="fluxwallet_test", account_id=1)
        wallet.new_key(account_id=acc.account_id, network="fluxwallet_test")
        wallet.new_key(account_id=acc.account_id, network="fluxwallet_test")
        ...
        self.assertEqual(wallet.balance(network="fluxwallet_test", account_id=0), 0)
        self.assertEqual(
            wallet.balance(network="fluxwallet_test", account_id=1), 600000000
        )
        ...
        t = wallet.send_to(
            "21EsLrvFQdYWXoJjGX8LSEGWHFJDzSs2F35",
            10000000,
            account_id=1,
            network="fluxwallet_test",
            fee=1000,
        ...
        self.assertTrue(t.verified)
        self.assertEqual(
            wallet.balance(network="fluxwallet_test", account_id=1), 589999000
        )
        self.assertEqual(
            len(wallet.transactions(account_id=0, network="fluxwallet_test")), 0
        )
        self.assertEqual(
            len(wallet.transactions(account_id=1, network="fluxwallet_test")), 7
        )
        del wallet

    def test_wallet_multi_networks_account_bip44_code_error(self):
        wlt = Wallet.create(
            "wallet-bip44-code-error", network="testnet", db_uri=self.DATABASE_URI
        )
        error_str = "Can not create new account for network litecoin_testnet with same BIP44 cointype"
        self.assertRaisesRegexp(
            WalletError, error_str, wlt.new_account, network="litecoin_testnet"
        )

    def test_wallet_get_account_defaults(self):
        w = wallet_create_or_open(
            "test_wallet_get_account_defaults",
            witness_type="segwit",
            db_uri=self.DATABASE_URI,
        )
...
@parameterized_class(*params)
class TestWalletfluxwalletTestnet(TestWalletMixin, unittest.TestCase):
    @classmethod
    def setUpClass(cls):
        cls.db_remove()

    def test_wallet_fluxwallet_testnet_sendto(self):
        w = Wallet.create(
            network="fluxwallet_test",
            name="test_wallet_fluxwallet_testnet_sendto",
            db_uri=self.DATABASE_URI,
        )
        ...
        self.assertIsNone(
            w.send_to(
                "21DBmFUMQMP7A6KeENXgZQ4wJdSCeGc2zFo", 5000000, offline=False
            ).error
        )
        self.assertIsNone(
            w.send_to(
                "21DBmFUMQMP7A6KeENXgZQ4wJdSCeGc2zFo", "10000 satTST", offline=False
            ).error
        )
        self.assertIsNone(
            w.send_to(
                "21DBmFUMQMP7A6KeENXgZQ4wJdSCeGc2zFo", Value("40 mTST"), offline=False
            ).error
        )

    def test_wallet_fluxwallet_testnet_send_utxos_updated(self):
        w = Wallet.create(
            network="fluxwallet_test",
            name="test_wallet_fluxwallet_testnet_send_utxos_updated",
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_fluxwallet_testnet_sendto_no_funds_txfee(self):
        w = Wallet.create(
            network="fluxwallet_test",
            name="test_wallet_fluxwallet_testnet_sendto_no_funds_txfee",
            db_uri=self.DATABASE_URI,
        )
        w.new_key()
        w.utxos_update()
        ...
        self.assertRaisesRegexp(
            ...
            w.send_to,
            "21DBmFUMQMP7A6KeENXgZQ4wJdSCeGc2zFo",
            balance,
            offline=False,
        )

    def test_wallet_fluxwallet_testnet_sweep(self):
        w = Wallet.create(
            network="fluxwallet_test",
            name="test_wallet_fluxwallet_testnet_sweep",
            db_uri=self.DATABASE_URI,
        )
        w.new_key()
        w.new_key()
        ...
        w.utxos_update()
        self.assertIsNone(
            w.sweep("21DBmFUMQMP7A6KeENXgZQ4wJdSCeGc2zFo", offline=False).error
        )
...
class TestWalletMultisig(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_multisig_2_wallets_private_master_plus_account_public(self):
        self.db_remove()
        pk1 = (
            "tprv8ZgxMBicQKsPdPVdNSEeAhagkU6tUDhUQi8DcCTmJyNLUyU7svTFzXQdkYqNJDEtQ3S2wAspz3K56CMcmMsZ9eXZ2nkNq"
            "gVxJhMHq3bGJ1X"
        )
        pk1_acc_pub = (
            "tpubDCZUk9HLxh5gdB9eC8FUxPB1AbZtsSnbvyrAAzsC8x3tiYDgbzyxcngU99rG333jegHG5vJhs11AHcSVkbwrU"
            "bYEsPK8vA7E6yFB9qbsTYi"
        )
        w1 = Wallet.create(
            name="test_wallet_create_1", keys=pk1, db_uri=self.DATABASE_URI
        )
        w2 = Wallet.create(
            name="test_wallet_create_2", keys=pk1_acc_pub, db_uri=self.DATABASE_URI
        )
        wk1 = w1.new_key()
        wk2 = w2.new_key()
    ...
    def test_wallet_multisig_create_2_cosigner_wallets(self):
        self.db_remove()
        pk_wif1 = (
            "tprv8ZgxMBicQKsPdvHCP6VxtFgowj2k7nBJnuRiVWE4DReDFojkLjyqdT8mtR6XJK9dRBcaa3RwvqiKFjsEQVhKfQmHZCCY"
            "f4jRTWvJuVuK67n"
        )
        pk_wif2 = (
            "tprv8ZgxMBicQKsPdkJVWDkqQQAMVYB2usfVs3VS2tBEsFAzjC84M3TaLMkHyJWjydnJH835KHvksS92ecuwwWFEdLAAccwZ"
            "KjhcA63NUyvDixB"
        )
        ...
        self.assertRaisesRegexp(
            WalletError, "Accounts are not supported for this wallet", wl1.account, 10
        )

    def test_wallet_multisig_fluxwallet_testnet_transaction_send(self):
        self.db_remove()
        ...
        wl.utxos_update()  # In fluxwallet_test network this generates new UTXO's
    ...
    def test_wallet_multisig_bitcoin_transaction_send_offline(self):
        ...
        key_list = [
            ...
            .public_master(multisig=True)
            .public(),
        ]
        wl = Wallet.create(
            "multisig_test_bitcoin_send",
            key_list,
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_multisig_bitcoin_transaction_send_no_subkey_for_path(self):
        ...
        key_list = [
            ...
            HDKey(
                "86b77aee5cfc3a55eb0b1099752479d82cb6ebaa8f1c4e9ef46ca0d1dc3847e6"
            ).public_master(multisig=True),
        ]
        wl = Wallet.create(
            "multisig_test_bitcoin_send2",
            key_list,
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_multisig_bitcoin_transaction_send_fee_priority(self):
        ...
        t2 = wl.transaction_create(
            [("3CuJb6XrBNddS79vr27SwqgR4oephY6xiJ", 100000)], fee="low"
        )
        t2.sign(pk2)
    ...
    def test_wallet_multisig_litecoin_transaction_send_offline(self):
        ...
        pk2 = HDKey(
            "e2cbed99ad03c500f2110f1a3c90e0562a3da4ba0cff0e74028b532c3d69d29d",
            network=network,
        )
        key_list = [
            HDKey(
                "e9e5095d3e26643cc4d996efc6cb9a8d8eb55119fdec9fa28a684ba297528067",
                network=network,
            ),
            pk2.public_master(multisig=True),
            HDKey(
    ...
    def test_wallet_multisig_2of2(self):
        ...
        msw1 = Wallet.create(
            "msw1",
            [keys[0], keys[1].subkey_for_path("m/45'").wif_public()],
            network="fluxwallet_test",
            sort_keys=False,
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        ...
        msw2 = Wallet.create(
            "msw2",
            [keys[0].subkey_for_path("m/45'").wif_public(), keys[1]],
            network="fluxwallet_test",
            sort_keys=False,
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        ...
        utxos = msw1.utxos()
        output_arr = [
            ("21KnydRNSmqAf8Py74mMiwRXYHGxW27zyDu", utxos[0]["value"] - 50000)
        ]
        input_arr = [
            (
                utxos[0]["txid"],
    ...
    def test_wallet_multisig_2of2_different_database(self):
        ...
        msw1 = Wallet.create(
            "msw1",
            [keys[0], keys[1].public_master(multisig=True)],
            network="fluxwallet_test",
            sort_keys=False,
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        ...
        msw2 = Wallet.create(
            "msw2",
            [keys[0].public_master(multisig=True), keys[1]],
            network="fluxwallet_test",
            sort_keys=False,
            sigs_required=2,
            db_uri=self.DATABASE_URI_2,
        )
        msw1.new_key()
    ...
    @classmethod
    def _multisig_test(
        cls, sigs_required, number_of_sigs, sort_keys, network, witness_type="legacy"
    ):
        # Create Keys
        key_dict = {}
        for key_id in range(number_of_sigs):
            key_dict[key_id] = HDKey(network=network, witness_type=witness_type)
        random_output_address = HDKey(
            network=network, witness_type=witness_type
        ).address()
        ...
        for wallet_id in range(number_of_sigs):
            ...
            for key_id in key_dict:
                if key_id == wallet_id:
                    ...
                    key_list.append(
                        key_dict[key_id].public_master(multisig=True, as_private=True)
                    )
    ...
    def test_wallet_multisig_2of3(self):
        self.db_remove()
        t = self._multisig_test(2, 3, False, "fluxwallet_test")
        self.assertTrue(t.verify())

    def test_wallet_multisig_2of3_segwit(self):
        self.db_remove()
        t = self._multisig_test(2, 3, False, "fluxwallet_test", "segwit")
        self.assertTrue(t.verify())

    def test_wallet_multisig_2of3_sorted(self):
        self.db_remove()
        t = self._multisig_test(2, 3, True, "fluxwallet_test")
        self.assertTrue(t.verify())

    def test_wallet_multisig_3of5(self):
        self.db_remove()
        t = self._multisig_test(3, 5, False, "fluxwallet_test")
        self.assertTrue(t.verify())

    def test_wallet_multisig_3of5_segwit(self):
        self.db_remove()
        t = self._multisig_test(3, 5, False, "fluxwallet_test", "segwit")
        self.assertTrue(t.verify())

    def test_wallet_multisig_2of2_with_single_key(self):
        self.db_remove()
        keys = [
            HDKey(network="fluxwallet_test"),
            HDKey(network="fluxwallet_test", key_type="single"),
        ]
        ...
        t = wl.transaction_create(
            [(HDKey(network="fluxwallet_test").address(), 6400000)], min_confirms=0
        )
        t.sign(keys[1])
        t.send()
        self.assertIsNone(t.error)
    ...
    def test_wallet_multisig_sorted_keys(self):
        ...
        w1 = Wallet.create(
            ...
            [
                key1,
                key2.public_master(multisig=True),
                key3.public_master(multisig=True),
            ],
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
        w2 = Wallet.create(
            "w2",
            [
                key1.public_master(multisig=True),
                key2,
                key3.public_master(multisig=True),
            ],
            sigs_required=2,
    ...
    def test_wallet_multisig_sign_with_external_single_key(self):
        ...
        wallet.utxos_update()
        wt = wallet.send_to(
            "21A6yyUPRL9hZZo1Rw4qP5G6h9idVVLUncE", 10000000, offline=False
        )
    ...
    def test_wallet_multisig_info(self):
        w = Wallet.create(
            "test_wallet_multisig_info",
            keys=[HDKey(network="fluxwallet_test"), HDKey(network="fluxwallet_test")],
            network="fluxwallet_test",
            cosigner_id=0,
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallets_multisig_missing_private_and_cosigner(self):
        k0 = (
            "xprv9s21ZrQH143K459uwGGCU3Wj3v1LFFJ42tgyTsNnr6p2BS6FZ9jQ7fmZMMnqsWSi2BBgpX3hFbR4ode8Jx58ibSNeaBLFQ68Xs3"
            "jwg4QFLh"
        )
        k1 = (
            "xpub661MyMwAqRbcGcJB4UPQpR2mUQtdUVbjjNC84DEK9ptZ2XgAC54U1onrH6tEMueYbzGPCRRPoDx5npvCS4Xryz8toVVEQ4ZFfkU"
            "cJobNZfn"
        )
        k2 = (
            "xpub661MyMwAqRbcFD9fBAsKxxRPgikEvig5KYT1CEnAp7FcfcFeu2ZxdNcj6DDUxNreWgftXody6NqDHmFmh8tRZ4UNAecucovtW4M"
            "bGjYRJFP"
        )
    ...
    def test_wallets_multisig_with_single_key_cosigner(self):
        ...
        w = wallet_create_or_open(
            "test_wallets_multisig_with_single_key_cosigner0",
            keys=[hdkey0, hdkey1, hdkey2],
            cosigner_id=0,
            db_uri=self.DATABASE_URI,
        )
        ...
        w2 = wallet_create_or_open(
            "test_wallets_multisig_with_single_key_cosigner2",
            keys=[hdkey0, hdkey1, hdkey2],
            cosigner_id=2,
            db_uri=self.DATABASE_URI,
        )
        ...
        del w
        w = wallet_create_or_open(
            "test_wallets_multisig_with_single_key_cosigner0", db_uri=self.DATABASE_URI
        )
        w.new_key(cosigner_id=2)
    ...
    def test_wallets_multisig_huge(self):
        for witness_type in ["legacy", "segwit"]:
            ...
            key_list = [
                HDKey(witness_type=witness_type, network=network)
                for _ in range(0, n_keys)
            ]
            pk_n = random.randint(0, n_keys - 1)
            key_list_cosigners = [
                k.public_master(multisig=True)
                for k in key_list
                if k is not key_list[pk_n]
            ]
    ...
    def test_wallets_multisig_huge_error(self):
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Redeemscripts with more then 15 keys are non-standard and could "
    ...
    def test_wallet_multisig_replace_sig_bug(self):
        ...
        key_list_cosigners = [
            k.public_master(multisig=True) for k in key_list if k is not key_list[0]
        ]
...
class TestWalletKeyImport(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_import_private_for_known_public(self):
        ...
        with Wallet.create(
            ...
            self.assertEqual(
                wlt.new_account().address, "16m3JAtQjHbmEZd8uYTyKebvrxh2RsFHB"
            )
            self.assertEqual(
                wlt.get_key().address, "1P8BTrsBn8DKGQq7nSWPiEiUDgiG8sW1kf"
            )

    def test_wallet_import_private_for_known_public_multisig(self):
        puk1 = (
            "Ltub2ULsQdDwsgJsxgNnetNNAz19RMhTzkZeG9sv8H3hyBuBkS4pMUJKcy3LsUETJfmS47mBg3mrxMuAYJqSf2jW3TyfP81WKHjc"
            "LV46vPh5kfX"
        )
        puk2 = (
            "Ltub2UZgTKsUVf6jmMZ3cihnLdvqxCCuJmAj2B3GaVUuQ6DPeNqFdtFAMr4rfbEkLveguPAKoiNfVNbWFexxxnxBUaZwXSWAp2X7"
            "kPMSdJ9rHRQ"
        )
        puk3 = (
            "Ltub2VcWC27VfGLB2PTwq3TQ5fsCAkyVE33WJ8QbBxPSQ8RWmsQmoWX1k1Fu4SfH2cVhKQCc5bPyUKEzEA35PeCmp5a9PCAWM26E"
            "AjuPh58gq7C"
        )
        prk3 = (
            "Ltpv71G8qDifUiNetjnY73hftMUMUdvPuiPGomJAfwEiJXbGFpWsMZJoZxwusqLSrgiW5kmjUempoLywoM4QfqGZWci2qx1omPQF"
            "iFEwaUv85GA"
        )

        with wallet_create_or_open(
            "mstest",
            [puk1, puk2, puk3],
            2,
            network="litecoin",
    ...
    def test_wallet_import_private_for_known_public_p2sh_segwit(self):
        pk1 = HDKey(
            "YXscyqNJ5YK411nwB3VjLYgjht3dqfKxyLdGSqNMGKhYdcK4Gh1CRSJyxS2So8KXSQrxtysS1jAmHtLnxRKa47xEiAx6hP"
            "vrj8tuEzyeR8TQNu5e"
        )
        pk2 = HDKey(
            "YXscyqNJ5YK411nwB4Jo3JCQ1GZNetf4BrLJjZiqdWKVzoXwPtyJ5xyNdZjuEWtqCeSZGtmg7SuQerERwniHLYL3aVcnyS"
            "ciEAxk7gLgDkoZC5Lq"
        )
        w = Wallet.create(
            "segwit-p2sh-p2wsh-import",
        ...
        self.assertTrue(t2.verify())
        t3 = w.sweep(
            "23CvEnQKsTVGgqCZzW6ewXPSJH9msFPsBt3", min_confirms=0, offline=False
        )
    ...
    def test_wallet_import_private_for_known_public_segwit_passphrases(self):
        ...
        wif1 = (
            "Zpub74zBTMSW6uSE3nFTJbxxvyF3P169wy9N3WzMzFBwWV56sj2NTsGEXAcj6HeLwAhsbs6Ca7Gx7JY1McV2CA1eANi2ubuQA34j"
            "CxbEPR7eLPm"
        )
        wif2 = (
            "Zpub75XfXxxDxRZPEiHJfGUtVWovyfCYT2DLkmPGCFjYdtChJ4r3VaUSbSrQKCVezZuZ6wiJ8UvuG3QPAJdKxB9iY9zUTZ9VLLMw"
            "jTF9ghGRx1Q"
        )
        ...
        w = Wallet.create(
            "mswlt", [p1, pubk2], db_uri=self.DATABASE_URI, witness_type=witness_type
        )
        wk = w.new_key()
        self.assertEqual(
        ...
        self.assertEqual(
            w.public_master(as_private=True)[1].wif,
        ...
        txids_expected = [
            "53b35eca3f2e767db02e4acc6c224d7a45f32158c8063f53c3d3660ab12d53ba",
...
class TestWalletTransactions(TestWalletMixin, unittest.TestCase, CustomAssertions):
    ...
    def setUpClass(cls):
        cls.db_remove()
        account_key = (
            "tpubDCmJWqxWch7LYDhSuE1jEJMbAkbkDm3DotWKZ69oZfNMzuw7U5DwEaTVZHGPzt5j9BJDoxqVkPHt2EpUF66FrZhpfq"
            "ZY6DFj6x61Wwbrg8Q"
        )
        cls.wallet = wallet_create_or_open(
            "utxo-test", keys=account_key, network="testnet", db_uri=cls.DATABASE_URI
        )
        cls.wallet.new_key()
    ...
    def test_wallet_sweep_public_wallet(self):
        tx = self.wallet.sweep(
            "mwCvJviVTzjEKLZ1UW5jaepjWHUeoYrEe7", fee_per_kb=50000, offline=False
        )
        prev_tx_list_check = [
    ...
    def test_wallet_offline_create_transaction(self):
        hdkey_wif = (
            "tprv8ZgxMBicQKsPf5exCdeBgnYjJt2LxDcQbv6u9HHymY3qh6EoTy8SGwou5xyvExL3iWfBsZWp3YUyo9gRmxQxrBS2FwGk"
            "qjcDhTcyVLhrXXZ"
        )
        ...
        t = wlt.transaction_create(
            [("n2S9Czehjvdmpwd2YqekxuUC1Tz5ZdK3YN", 1000)], fee=5000
        )
        t.sign()
        self.assertTrue(t.verify())
        del wlt
    ...
    def test_wallet_scan(self):
        ...
        wallet = wallet_create_or_open(
            "scan-test", keys=account_key, network="testnet", db_uri=self.DATABASE_URI
        )
        ...
        w = wallet_create_or_open(
            "fix-multiple-tx-1-block", keys=address, db_uri=self.DATABASE_URI
        )
        w.scan()
        self.assertEqual(
            w.transactions()[0].txid,
            "bae05e65c13a1b1635abf581a6250a458cbd672c914e2563b5bb175274f9c5a7",
        )
    ...
    def test_wallet_scan_utxos(self):
        ...
        exp_tx_list = [
            "3c634d82d6a9e855f26266acefb7096afdad80cad007079028bc259dc2f642a4",
    ...
    def test_wallet_two_utxos_one_key(self):
        wlt = Wallet.create(
            "double-utxo-test", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        key = wlt.new_key()
        wlt.utxos_update()
        utxos = wlt.utxos()
        ...
        inp1 = Input(
            ...
            output_n=utxos[0]["output_n"],
            keys=key.key_public,
            network="fluxwallet_test",
        )
        inp2 = Input(
            prev_txid=utxos[1]["txid"],
            output_n=utxos[1]["output_n"],
            keys=key.key_public,
            network="fluxwallet_test",
        )
        out = Output(10000000, address=key.address, network="fluxwallet_test")

        t = TransactionBuilder(inputs=[inp1, inp2], outputs=[out], network="testnet")
    ...
    def test_wallet_balance_update(self):
        wlt = Wallet.create(
            "test-balance-update", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
        ...
        self.assertEqual(
            t.txid, wlt.transaction_spent(t.inputs[0].prev_txid, t.inputs[0].output_n)
        )
        self.assertEqual(
            t.txid,
            wlt.transaction_spent(
                t.inputs[0].prev_txid.hex(), t.inputs[0].output_n_int
            ),
        )
        del wlt
    ...
    def test_wallet_balance_update_multi_network(self):
        k = (
            "tpubDCutwJADa3iSbFtB2LgnaaqJgZ8FPXRRzcrMq7Tms41QNnTV291rpkps9vRwyss9zgDc7hS5V1aM1by8nFip5VjpGpz1oP54peKt"
            "hJzfabX"
        )
        wlt = Wallet.create(
            "test_wallet_balance_update_multi_network",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        wlt.new_key()
        ...
        self.assertEqual(wlt.balance(network="fluxwallet_test"), 400000000)
        del wlt
    ...
    def test_wallet_balance_update_total(self):
        ...
        wlt = Wallet.create(
            "test_wallet_balance_update_total",
            keys=k,
            network="testnet",
            db_uri=self.DATABASE_URI,
        )
    ...
    def test_wallet_add_dust_to_fee(self):
        # Send fluxwallet test transaction and check if dust resume amount is added to fee
        wlt = Wallet.create(
            "bcltestwlt", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transactions_send_update_utxos(self):
        # Send fluxwallet test transaction and check if all utxo's are updated after send
        wlt = Wallet.create(
            "bcltestwlt2", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
    ...
    def test_wallet_transaction_import(self):
        wlt = Wallet.create(
            "bcltestwlt3", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_raw(self):
        wlt = Wallet.create(
            "bcltestwlt4", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
        t = wlt.send_to(to_key.address, 50000000)
        t2 = wlt.transaction_import_raw(t.raw())
        self.assertDictEqualExt(
            t.as_dict(), t2.as_dict(), ["spending_txid", "spending_index_n"]
        )
        del wlt

    def test_wallet_transaction_import_raw_locktime(self):
        wlt = Wallet.create(
            "bcltestwlt4b", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_import_raw_segwit_fee(self):
        ...
        pk = (
            "YXscyqNJ5YK411nwB2peYdMeJPmkJmMJCfNdo9JuWkEKLZhVSoUjbRRinVqqtBN2GNC2A6L1Taz1e3LWApHkC84GgTp3vr7neD"
            "ZTxXnvGkUwVz4c"
        )
        wlt = Wallet.create(
            "bcltestwlt-size",
    ...
    def test_wallet_transaction_import_dict(self):
        wlt = Wallet.create(
            "bcltestwlt5", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_fee_limits(self):
        wlt = Wallet.create(
            "bcltestwlt6", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        to_key = wlt.get_key()
        wlt.utxos_update()
        self.assertRaisesRegexp(
            WalletError,
            "Fee per kB of 666 is lower then minimal network fee of 1000",
            wlt.send_to,
            to_key.address,
            50000000,
            fee=150,
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Fee per kB of 1333333 is higher then maximum network fee of 1000000",
            wlt.send_to,
            to_key.address,
            50000000,
            fee=300000,
    ...
    def test_wallet_transaction_fee_zero_problem(self):
        wlt = Wallet.create(
            name="bcltestwlt7", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        nk = wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transactions_estimate_size(self):
        ...
        for witness_type in ["legacy", "p2sh-segwit", "segwit"]:
            wallet_delete_if_exists(
                "wallet_estimate_size", force=True, db_uri=self.DATABASE_URI
            )
            wl3 = Wallet.create(
                "wallet_estimate_size",
                witness_type=witness_type,
                db_uri=self.DATABASE_URI,
            )
            wl3.utxo_add(wl3.get_key().address, 110000, prev_txid, 0, 1)
            to_address = wl3.get_key_change().address
            t = wl3.transaction_create(
                [(to_address, 90000)], fee=10000, random_output_order=False
            )
            t.estimate_size()
            size1 = t.size
            t.sign()
        ...
        for witness_type in ["legacy", "p2sh-segwit", "segwit"]:
            ...
            wallet_delete_if_exists(
                "wallet_estimate_size_multisig", force=True, db_uri=self.DATABASE_URI
            )
            wl3 = Wallet.create(
                "wallet_estimate_size_multisig",
                [p1, p2.public_master(), p3.public_master()],
                sigs_required=2,
                db_uri=self.DATABASE_URI,
            )
            wl3.utxo_add(wl3.get_key().address, 110000, prev_txid, 0, 1)
    ...
    def test_wallet_transaction_method(self):
        pk1 = HDKey(network="fluxwallet_test")
        pk2 = HDKey(network="fluxwallet_test")
        w = Wallet.create(
    ...
    def test_wallet_transaction_from_txid(self):
        w = Wallet.create(
            "testwltbcl",
            keys="dda84e87df25f32d73a7f7d008ed2b89fc00d9d07fde588d1b8af0af297023de",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        w.utxos_update()
        ...
        txid = wts[0].txid
        self.assertEqual(
            txid, "86eebbefb1062b45b19bc1bbc3fbe044fadcf592dc4e64f1a13a58ac362123ef"
        )
    ...
    def test_wallet_transaction_sign_with_hex(self):
        k = HDKey(network="fluxwallet_test")
        pmk = k.public_master()
        w = Wallet.create(
            "wallet_tx_tests",
            keys=pmk,
    ...
    def test_wallet_transaction_sign_with_wif(self):
        wif = (
            "YXscyqNJ5YK411nwB4eU6PmyGTJkBUHjgXEf53z4TTjHCDXPPXKJD2PyfXonwtT7VwSdqcZJS2oeDbvg531tEsx3yq4425Mfrb9aS"
            "PyNQ5bUGFwu"
        )
        wif2 = (
            "YXscyqNJ5YK411nwB4UK8ScMahPWewyKrTBjgM5BZKRkPg8B2HmKT3r8yc2GFg9GqgFXaWmxkTRhNkRGVxbzUREMH8L5HxoKGCY8"
            "WDdf1GcW2k8q"
        )
        w = wallet_create_or_open(
            "test_wallet_transaction_sign_with_wif",
        ...
        w.utxos_update()
        t = w.send_to(
            "blt1q285vnphcs4r0t5dw06tmxl7aryj3jnx88duehv4p7eldsshrmygsmlq84z",
            2000,
            fee=1000,
            offline=False,
        )
    ...
    def test_wallet_transaction_restore_saved_tx(self):
        w = wallet_create_or_open(
            "test_wallet_transaction_restore",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        wk = w.get_key()
        if not USE_FASTECDSA:
            self.skipTest(
                "Need fastecdsa module with deterministic txid's to run this test"
            )
        utxos = [
            {
                "address": wk.address,
    ...
    def test_wallet_transaction_send_keyid(self):
        ...
        w.utxos_update()
        t = w.send_to(
            "blt1qtk5swtntg8gvtsyr3kkx3mjcs5ncav84exjvde",
            150000000,
            input_key_id=keys[1].key_id,
            offline=False,
        )
    ...
    def test_wallet_transactions_update_by_txids(self):
        ...
        w.transactions_update_by_txids(
            ["b246dad8980093fa55814b4739396fd6a7f5f28994f721f55d1a862d6c98e7ab"]
        )
        txs = w.transactions()
    ...
    def test_wallet_transactions_sign_with_mnenomic(self):
        ...
        prv_key = HDKey.from_passphrase(phr, network="fluxwallet_test")
        w = wallet_create_or_open(
            "wlt-sign-with-mnemonic",
            keys=prv_key.public(),
            network="fluxwallet_test",
            scheme="single",
            db_uri=self.DATABASE_URI,
    ...
    def test_wallet_select_inputs(self):
        wlt = Wallet.create(
            "bclwltsltinputs", network="fluxwallet_test", db_uri=self.DATABASE_URI
        )
        wlt.get_key()
        wlt.utxos_update()
    ...
    def test_wallet_transaction_create_exceptions(self):
        wif = (
            "YXscyqNJ5YK411nwB3kahnuWqF2KUJfJNRGG1n3zLPi3MSPCRcD2cmcVz1UKBjTMuMxAfXrbSAe5qJfu5nnSuRZKEtqJt"
            "FwNjcNknbseoKp1vR2h"
        )
        wlt = Wallet.create(
            "test_wallet_transaction_create_exceptions",
            keys=wif,
            db_uri=self.DATABASE_URI,
        )
        wlt.utxos_update()
        self.assertRaisesRegexp(
            ...
            wlt.transaction_create,
            "217rBycQpv9rjhiLcg94vdZ7muMVLJ9yysJ",
        )
        inps = wlt.select_inputs(150000000)
        self.assertRaisesRegexp(
            WalletError,
            "Input array contains 2 UTXO's but max_utxos=1 parameter specified",
            wlt.transaction_create,
            [("217rBycQpv9rjhiLcg94vdZ7muMVLJ9yysJ", 150000000)],
            inps,
            max_utxos=1,
        )
        wallet_empty(
            "test_wallet_transaction_create_exceptions", db_uri=self.DATABASE_URI
        )
        self.assertRaisesRegexp(
            WalletError,
            "UTXO",
    ...
    def test_wallet_transactions_sweep(self):
        w = wallet_create_or_open(
            "test_wallet_sweep_check_fee", db_uri=self.DATABASE_URI
        )
        w.utxo_add(
            w.new_key().address,
            5000,
            "f31446151f06522eb321d5992f4f1c95123c8b9d082b92c391df83c6d0a35516",
            0,
            1,
        )
        ...
        self.assertRaisesRegex(
            WalletError,
            "Amount to send is smaller then dust amount: 1000",
            w.sweep,
            to_address="14pThTJoEnQxbJJVYLhzSKcs6EmZgShscX",
            fee=4000,
        )
        self.assertRaisesRegex(
            WalletError,
    ...
    def test_wallet_sweep_multiple_inputs_or_outputs(self):
        w = wallet_create_or_open(
            "test_wallet_sweep_multiple_outputs", db_uri=self.DATABASE_URI
        )
        ...
        t = w.sweep(
            [
                ("14pThTJoEnQxbJJVYLhzSKcs6EmZgShscX", 11000),
                ("1GSffHyTGyKvQWpKHc7Mjd8K2bmbt7g9Xx", 0),
            ],
            offline=True,
            fee=2000,
        )
        ...
        input_key_ids = [
            u["key_id"] for u in w.utxos() if u["value"] in [50000, 25000, 100000]
        ]
        t = w.sweep(
            [
                ("14pThTJoEnQxbJJVYLhzSKcs6EmZgShscX", 150000),
                ("1GSffHyTGyKvQWpKHc7Mjd8K2bmbt7g9Xx", 0),
            ],
    ...
    def test_wallet_send_multiple_change_outputs(self):
        w = wallet_create_or_open(
            "test_wallet_send_multiple_change_outputs", db_uri=self.DATABASE_URI
        )
        ...
        t = w.send_to(
            "1D6kjUgadFdpvEL7hUUDsmqSemeUaN1iFi",
            150000,
        ...
        self.assertFalse(
            [o.value for o in t.outputs if o.value < o.network.dust_amount]
        )
        self.assertTrue(t.verified)
    ...
    def test_wallet_transaction_save_and_load(self):
        w = wallet_create_or_open(
            "wallet_transaction_save_and_load",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        w.utxos_update()
    ...
    def test_wallet_transaction_save_and_load_filename(self):
        w = wallet_create_or_open(
            "wallet_transaction_save_and_load_filename",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        w.utxos_update()
    ...
    def test_wallet_avoid_forced_address_reuse(self):
        w = wallet_create_or_open(
            "wallet_avoid_forced_address_reuse_test",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        k1 = w.get_key()
    ...
    def test_wallet_avoid_forced_address_reuse2(self):
        w = wallet_create_or_open(
            "wallet_avoid_forced_address_reuse_test2", db_uri=self.DATABASE_URI
        )
        utxos = [
            {
                "address": w.new_key().address,
        ...
        self.assertRaisesRegexp(
            ...
            "",
            w.send_to,
            "bc1qx76mfmrgvejprscpk8e76d90h94xdhhgnr3jfk",
            200001,
            fee=150,
        )
    ...
    def test_wallet_create_import_key(self):
        w = wallet_create_or_open(
            "test_wallet_create_import_key_private",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        ...
        wk = w.public_master()
        w2 = wallet_create_or_open(
            "test_wallet_create_import_key_public",
            network="fluxwallet_test",
            keys=wk,
            db_uri=self.DATABASE_URI,
        )
...
class TestWalletDash(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_import_dash(self):
        ...
        self.assertEqual(newkey.address, "XtVa6s1rqo9BNXir1tb6KEXsj5NGogp1QB")
    ...
    def test_wallet_multisig_dash(self):
        ...
        wl1 = Wallet.create(
            "multisig_test_wallet1",
            [pk1, pk2.public_master(multisig=True)],
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
        wl2 = Wallet.create(
            "multisig_test_wallet2",
            [pk1.public_master(multisig=True), pk2],
            sigs_required=2,
            db_uri=self.DATABASE_URI,
        )
        wl1_key = wl1.new_key()
    ...
    def test_wallet_import_private_for_known_public_multisig_dash(self):
        ...
        with wallet_create_or_open(
            "mstest_dash",
            [
                pk1.public_master(multisig=True),
                pk2.public_master(multisig=True),
                pk3.public_master(multisig=True),
            ],
            2,
            network=network,
            sort_keys=False,
    ...
    def test_wallet_merge_transactions(self):
        w = wallet_create_or_open(
            "wallet_merge_transactions_tests",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
        )
        w.utxos_update()
...
class TestWalletSegwit(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_segwit_create_pswsh(self):
        ...
        self.assertEqual(
            w.get_key().address,
            "bc1qfjhmzzt9l6dmm0xx3tc6qrtff8dve7j7qrcyp88tllszm97r84aqxel5jk",
        )
    ...
    def test_wallet_segwit_create_p2sh_p2wpkh(self):
        ...
        self.assertEqual(
            Address(b"\x00\x14" + k1.key().hash160, script_type="p2sh").address, address
        )
        self.assertEqual(
            Address(k1.key().public_byte, script_type="p2sh_p2wpkh").address, address
        )
    ...
    def test_wallet_segwit_p2wpkh_send(self):
        ...
        w.utxos_update()
        t = w.send_to(
            "blt1q7ywlg3lsyntsmp74jh65pnkntk3csagdwpz78k", 10000, offline=False
        )
    ...
    def test_wallet_segwit_p2wsh_send(self):
        ...
        w.utxos_update(
            key_id=k.key_id
        )  # Test db updates after second request and only update single key
        t = w.send_to(
            "blt1q7r60he62p52u6h9zyxl6ew4dmmshpmk5sluaax48j9c7zyxu6m0smrjqxa",
            10000,
            offline=False,
        )
        self.assertEqual(t.witness_type, "segwit")
    ...
    def test_wallet_segwit_uncompressed_error(self):
        k = HDKey(compressed=False, network="fluxwallet_test")
        self.assertRaisesRegexp(
            BKeyError,
            "Uncompressed keys are non-standard",
            wallet_create_or_open,
            "segwit_uncompressed_error",
            k,
            witness_type="segwit",
            network="fluxwallet_test",
            db_uri=self.DATABASE_URI,
    ...
    def test_wallet_segwit_bitcoin_send(self):
        ...
        t = wl1.transaction_create(
            [(to_address, 100000)], fee=10000, random_output_order=False
        )
        ...
        wl3 = Wallet.create(
            "segwit_bitcoin_p2sh_p2wpkh_send",
            witness_type="p2sh-segwit",
            db_uri=self.DATABASE_URI,
        )
        wl3.utxo_add(wl3.get_key().address, 110000, prev_txid, 0, 1)
    ...
    def test_wallet_segwit_litecoin_multi_accounts(self):
        phrase = (
            "rug rebuild group coin artwork degree basic humor flight away praise able"
        )
        w = Wallet.create(
            "segwit_wallet_litecoin_p2wpkh",
            keys=phrase,
            network="litecoin",
            db_uri=self.DATABASE_URI,
            witness_type="segwit",
        ...
        self.assertEqual(
            w.get_key().address, "ltc1qsrzxzg39jyt8knsw5hlqmpwmuc8ejxvp9hfch8"
        )
        self.assertEqual(
            w.get_key_change().address, "ltc1q9n6zknsw2hhq7dkyvczars8vl8zta5yusjjem5"
        )
        acc2 = w.new_account()
        btc_acc = w.new_account(network="bitcoin")
        self.assertEqual(
            w.get_key(acc2.account_id).address,
            "ltc1quya06p0ywk55rvf6jjpvxwmd66n2axu8qhnned",
        )
        self.assertEqual(
            w.get_key(btc_acc.account_id, network="bitcoin").address,
        ...
        w = Wallet.create(
            "segwit_wallet_litecoin_p2sh_p2wpkh",
            keys=phrase,
            network="litecoin",
            db_uri=self.DATABASE_URI,
            witness_type="p2sh-segwit",
        ...
        self.assertEqual(
            w.get_key_change().address, "MWQoYMDTNvwZPNNypLMzkQ7JNSCtvS554j"
        )

    def test_wallet_segwit_litecoin_sweep(self):
        phrase = (
            "wagon tunnel garage blast eager jaguar shop bring lake dumb chalk emerge"
        )
        w = wallet_create_or_open(
            "ltcsw",
            phrase,
        ...
        w.utxo_add(
            "ltc1qu8dum66gd6dfr2cchgenf87qqxgenyme2kyhn8",
            28471723,
            "21da13be453624cf46b3d883f39602ce74d04efa7a186037898b6d7bcfd405ee",
            10,
            99,
        )
    ...
    def test_wallet_segwit_litecoin_multisig(self):
        p1 = (
            "only sing document speed outer gauge stand govern way column material odor"
        )
        ...
        w.get_keys(number_of_keys=2)
        w.utxo_add(
            "ltc1qkewaz7lxn75y6wppvqlsfhrnq5p5mksmlp26n8xsef0556cdfzqq2uhdrt",
            2100000000000001,
            "21da13be453624cf46b3d883f39602ce74d04efa7a186037898b6d7bcfd405ee",
            0,
            15,
    ...
    def test_wallet_segwit_multisig_multiple_inputs(self):
        main_key = HDKey(network="fluxwallet_test")
        cosigner = HDKey(network="fluxwallet_test")
        w = wallet_create_or_open(
            "test_wallet_segwit_multisig_multiple_inputs",
    ...
    def test_wallet_segwit_multiple_account_paths(self):
        ...
        w = Wallet.create(
            "account-test",
            keys=[pk1, pk2.public_master(multisig=True)],
            witness_type="segwit",
            db_uri=self.DATABASE_URI,
        )
        w.new_account()
        ...
        self.assertRaisesRegexp(
            WalletError,
            "Account with ID 100 already exists for this wallet",
            w.new_account,
            "test",
            100,
        )
        self.assertRaisesRegexp(
            WalletError,
            "Account with ID 1001 not found in this wallet",
            w.account,
            1001,
        )
    ...
    def test_wallet_segwit_multiple_networks_accounts(self):
        ...
        self.assertListEqual(
            sorted([nw.name for nw in wallet.networks()]), networks_expected
        )
        self.assertListEqual(
            [k.path for k in wallet.keys_accounts(network="litecoin")],
            ["m/48'/2'/0'/2'", "m/48'/2'/1'/2'"],
        )
        self.assertEqual(
            wallet.keys(network="litecoin")[0].address,
            "MQNA8FYrN2fvD7SSYny3Ccvpapvsu9cVJH",
        )
        self.assertEqual(
            wallet.keys(network="bitcoin")[0].address,
            "3L6XFzC6RPeXSFpZS8v4S86v4gsNmKFnFT",
        )
...
class TestWalletKeyStructures(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_path_expand(self):
        wlt = wallet_create_or_open(
            "wallet_path_expand", network="bitcoin", db_uri=self.DATABASE_URI
        )
        self.assertListEqual(wlt.path_expand([8]), ["m", "44'", "0'", "0'", "0", "8"])
        self.assertListEqual(wlt.path_expand(["8"]), ["m", "44'", "0'", "0'", "0", "8"])
        self.assertListEqual(
            wlt.path_expand(["99'", 1, 2]), ["m", "44'", "0'", "99'", "1", "2"]
        )
        self.assertListEqual(
            wlt.path_expand(["m", "purpose'", "coin_type'", "1'", 2, 3]),
            ["m", "44'", "0'", "1'", "2", "3"],
        )
        self.assertListEqual(
            wlt.path_expand(["m", "purpose'", "coin_type'", "1", 2, 3]),
        ...
        self.assertRaisesRegexp(
            BKeyError,
            "Please provide value for 'address_index'",
            wlt.path_expand,
            ["m", 45, "cosigner_index", 55, "address_index"],
        )
        self.assertRaisesRegexp(
            BKeyError,
            "Variable bestaatnie not found in Key structure definitions in main.py",
            wlt.path_expand,
            ["m", "bestaatnie'", "coin_type'", "1", 2, 3],
        )

    def test_wallet_exotic_key_paths(self):
        w = Wallet.create(
            "simple_custom_keypath",
            key_path="m/change/address_index",
            db_uri=self.DATABASE_URI,
        )
        ...
        wif1 = (
            "Zpub74CSuvLPQxWkdW7bivQAhomXZTzbE8quAakKRg1C3x7uDcCCeh7zPp1tZrtJrscihJRASZWjZQ7nPQj1SHTn8gkzAHPZL3dC"
            "MbMQLFwMKVV"
        )
        wif2 = (
            "Zpub75J84sqDUenYwh6eYwFnpXmfRMkfCwyEUBsN6fkGLQhh4nGmdxHw1io3AcUvAcK14RXosXfjG6Gfkz3NUHCa1JESGCf52ZWQ"
            "d2CqDgo1rLa"
        )
        w = Wallet.create(
            "long_key_path",
    ...
    def test_wallet_normalize_path(self):
        self.assertEqual(
            normalize_path("m/48h/0p/100H/1200'/1234555"), "m/48'/0'/100'/1200'/1234555"
        )
        self.assertRaisesRegexp(
            WalletError,
            "Could not parse path. Index is empty.",
            normalize_path,
            "m/44h/0p/100H//1201",
        )
...
class TestWalletReadonlyAddress(TestWalletMixin, unittest.TestCase):
    ...
    def test_wallet_readonly_create_and_import(self):
        ...
        self.assertListEqual(
            ...
            [
                ...
                "12yuSkjKmHzXCFn39PK1XP3XyeoVw9LJdN",
            ],
        )
        ...
        self.assertRaisesRegexp(
            ...
            "No unspent",
            w.send_to,
            "1ApcyGtcX4DUmfGqPBPY1bvKEh2irLqnhp",
            50000,
        )
        self.assertEqual(
            w.utxo_last("13A1W4jLPP75pzvn2qJ5KyyqG3qPSpb9jM"),
            "22965c151d850b5c7d98561e5432459141b1321a299cdcac1764f540eef56451",
        )

    def test_wallet_address_import_public_key(self):
        wif = (
            "xpub661MyMwAqRbcFCwFkcko75u2VEinbG1u5U4nq8AFJq4AbLPEvwcmhZGgGcnDcEBpcfAFEP8vVhbJJvX1ieGWdoaa5AnHfyB"
            "DAY95TfYH6H6"
        )
        address = "1EJiPa66sT4PCDCFnc7oRnpWebAogPqppr"
        w = Wallet.create(
            "test_wallet_address_import_public_key", address, db_uri=self.DATABASE_URI
        )
        self.assertEqual(w.addresslist(), [address])
        self.assertIsNone(w.main_key.key_public)
        w.import_key(wif)
        self.assertEqual(
            w.main_key.key_public.hex(),
    ...
    def test_wallet_address_import_public_key_segwit(self):
        ...
        wif = (
            "zprvAWgYBBk7JR8Gj9CNFRBUq3DvNHDbZhH4L6AybFSTCH7DDW6boPyiQfTigAPhJma5wC4TP1o53Gz1XLh94xD3dVQUpsFDaCb2"
            "9XmDQKBwKhz"
        )
        w = Wallet.create(
            "test_wallet_address_import_public_key_segwit",
            address,
            db_uri=self.DATABASE_URI,
        )
        self.assertEqual(w.addresslist(), [address])
    ...
    def test_wallet_address_import_private_key(self):
        wif = (
            "xprv9s21ZrQH143K2irnebDnjwxHwCtJBoJ3iF9C2jkdkVXBiY46PQJX9kxCRMVcM1YXLERWUiUBoxQEUDqFAKbrTaL9FB4HfRY"
            "jsGgCGpRPWTy"
        )
        address = "1EJiPa66sT4PCDCFnc7oRnpWebAogPqppr"
        w = Wallet.create(
            "test_wallet_address_import_private_key", address, db_uri=self.DATABASE_URI
        )
