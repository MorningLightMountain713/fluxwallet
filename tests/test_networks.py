...
#    Unit Tests for fluxwallet Network Class
...
class TestNetworks(unittest.TestCase):
    ...
    def test_networks_prefix_bech32_network_by_value_sorted(self):
        self.assertEqual(
            network_by_value("prefix_bech32", "ltc"), ["litecoin", "litecoin_legacy"]
        )
    ...
    def test_networks_prefix_hdkey_wif(self):
        ...
        self.assertRaisesRegex(
            NetworkError,
            "WIF Prefix for script type p2wpkh not found",
            Network("dash").wif_prefix,
            witness_type="segwit",
        )
    ...
    def test_networks_print_value(self):
        ...
        self.assertEqual(
            print_value(123, rep="symbol", denominator=0.001), "0.00123 m₿"
        )
        self.assertEqual(print_value(123, denominator=1e-6), "1.23 µBTC")
        self.assertEqual(
            print_value(1e14, network="dogecoin", denominator=1e6, decimals=0),
            "1 MDOGE",
        )
        self.assertEqual(
    ...
    def test_network_defined(self):
        ...
        self.assertRaisesRegexp(
            NetworkError,
            "Network bitcoiin not found in network definitions",
            Network,
            "bitcoiin",
        )
    ...
    def test_wif_prefix_search(self):
        ...
        self.assertEqual(
            wif_prefix_search("0488ADE4", network="bitcoin", multisig=False)[0],
            exp_dict,
        )
        self.assertEqual(
            wif_prefix_search(
                "lettrythisstrangestring", network="bitcoin", multisig=False
            ),
            [],
        )
