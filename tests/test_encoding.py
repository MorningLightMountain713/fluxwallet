...
from fluxwallet.encoding import _bech32_polymod, _codestring_to_array
...
class TestEncodingMethodsChangeBase(unittest.TestCase):
    ...
    def test_change_base_b58_dec(self):
        self.assertEqual(
            5283658277747592673868818217239156372404875337009783985623,
            change_base("1LeNnaRtV52nNtZXvtw6PaGKpk46hU1Xmx", 58, 10),
        )

    def test_change_base_b58_bin(self):
        self.assertEqual(
            b"\x00\xd7{\xf7b\x8c\x19\xe6\x99\x01\r)xz)\xaf\xcf\x8e\x92\xadZ\x05=U\xd7",
            change_base("1LeNnaRtV52nNtZXvtw6PaGKpk46hU1Xmx", 58, 256),
        )

    def test_change_base_b58_hex(self):
        self.assertEqual(
            "00D77BF7628C19E699010D29787A29AFCF8E92AD5A053D55D7",
            change_base("1LeNnaRtV52nNtZXvtw6PaGKpk46hU1Xmx", 58, 16).upper(),
        )

    def test_change_base_dec_b58(self):
        self.assertEqual(
            "LeNnaRtV52nNtZXvtw6PaGKpk46hU1Xmx",
            change_base(
                "5283658277747592673868818217239156372404875337009783985623", 10, 58, 33
            ),
        )
    ...
    def test_change_base_encodings_bytes(self):
        self.assertEqual(
            ...
            change_base(
                b"LR\x12zr\xfbB\xb8$9\xab\x18i}\xcf\xcf\xb9j\xc6;\xa8 \x983\xb2\xe2\x9f#\x02\xb8"
    ...
    def test_change_base_encodings_utf8(self):
        self.assertEqual(
            "e782ba20e5ae8b20e69ab420e6b2bb20e4bcaf20e58f8a20e7819820e586b620e5bf9920e9808320e6b99820e8"
    ...
    def test_change_base_bytes_as_string(self):
        ...
        self.assertEqual(
            "c8e90996c7c6080ee06284600c684ed904d14c5c", change_base(s, 256, 16)
        )

    def test_change_base_decimal_input_lenght_exception(self):
        self.assertRaisesRegexp(
            EncodingError,
            "For a decimal input a minimum output length is required",
            change_base,
            100,
            10,
            2,
    ...
    def test_encoding_exceptions(self):
        self.assertRaisesRegexp(
            EncodingError, "Unknown input format {}", change_base, {}, 4, 2
        )
        self.assertRaisesRegexp(
            EncodingError,
            "Byteint must be a list or defined as bytes",
            varbyteint_to_int,
            "fd1027",
        )
        self.assertRaisesRegexp(
            EncodingError, "Input must be a number type", int_to_varbyteint, "1000"
        )
        self.assertRaisesRegexp(
            TypeError, "String value expected", normalize_string, 100
        )
        self.assertRaisesRegexp(
            EncodingError,
            "Encoding base32 not supported",
            pubkeyhash_to_addr,
            "123",
            encoding="base32",
        )
        ...
        self.assertRaisesRegexp(
            EncodingError,
            "Invalid bech32 address. Prefix 'qc', prefix expected is 'bc'",
...
class TestEncodingMethodsAddressConversion(unittest.TestCase):
    ...
    def test_pkh_to_addr_conversion_1(self):
        self.assertEqual(
            "1LkthjzqGyhAWAmA9Dgbyp9pNMBXQj9ZZ3",
            pubkeyhash_to_addr("d8b76f6dd0e8d17cd34c3703ad5a120ba83ff857"),
        )

    def test_pkh_to_addr_conversion_2(self):
        self.assertEqual("1111111111111111111114oLvT2", pubkeyhash_to_addr("00" * 20))

    def test_address_to_pkh_bech32(self):
        addr = "bc1qy8qmc6262m68ny0ftlexs4h9paud8sgce3sf84"
        self.assertEqual(
            addr_to_pubkeyhash(addr),
            b"!\xc1\xbciZV\xf4y\x91\xe9_\xf2hV\xe5\x0fx\xd3\xc1\x18",
        )
        self.assertEqual(
            addr_to_pubkeyhash(addr, as_hex=True),
            "21c1bc695a56f47991e95ff26856e50f78d3c118",
        )

    def test_pkh_to_bech32_address(self):
        addr = pubkeyhash_to_addr(
            "45d093a97d5710c80363c69618e826efad42edb1", encoding="bech32"
        )
        self.assertEqual(addr, "bc1qghgf82ta2ugvsqmrc6tp36pxa7k59md3czjhjc")

    def test_address_base58_zero_prefixes(self):
        self.assertEqual(
            pubkeyhash_to_addr_base58("00003acd8f60b766e48e9db32093b419c21de7e9"),
            "111GxfgFVyDW3zcFpUF1upSZoL7GCRiLk",
        )
        self.assertEqual(
            change_base("000000003acd8f60b766e48e9db32093b419c21de7e9b35f7e0d", 16, 58),
            "1111GxfgFVyDW3zcFpUF1upSZoL7GCRiLk",
        )
        self.assertEqual(
            change_base("0000003acd8f60b766e48e9db32093b419c21de7e9b35f7e0d", 16, 58),
            "111GxfgFVyDW3zcFpUF1upSZoL7GCRiLk",
        )
        self.assertEqual(
            change_base("1111GxfgFVyDW3zcFpUF1upSZoL7GCRiLk", 58, 256).hex(),
            "000000003acd8f60b766e48e9db32093b419c21de7e9b35f7e0d",
        )
        self.assertRaisesRegexp(
            EncodingError,
            "Invalid address hash160 length, should be 25 characters not",
...
class TestEncodingMethodsStructures(unittest.TestCase):
    ...
    def test_varbyteint_to_int_3(self):
        self.assertEqual(
            18440744073009551600,
            varbyteint_to_int(b"\xff\xf0\xd8\x9f\xf9\x07\xaf\xea\xff")[0],
        )
    ...
    def test_int_to_varbyteint_3(self):
        self.assertEqual(
            ...
            int_to_varbyteint(18446744073709551615),
        )

    def test_varstr(self):
        self.assertEqual(
            b"\x1eThis string has a length of 30",
            varstr("This string has a length of 30"),
        )

    def test_convert_der_sig(self):
        sig = (
            b"0E\x02!\x00\xe7\x1a\x8d\xd8>y\xfb\xd6/r\xa3\xd0\xd8\xa8\x1f\xdd\xbaS[\xd0\xf0\x88\xfa\x8b\xe1L"
            b"\xd3F\x7f\xe5\x17\xae\x02 _l\xa4\x89LS\xcd\x8em&\xf7\x99uN\xb6\xfc\x0e\x86\xf6\x12\xd6\xdejL|"
            b"\x07\xdcX \xa0\xe5\x18"
        )
        self.assertEqual(
            "e71a8dd83e79fbd62f72a3d0d8a81fddba535bd0f088fa8be14cd3467fe517ae5f6ca4894c53cd8e6d26f"
            "799754eb6fc0e86f612d6de6a4c7c07dc5820a0e518",
            convert_der_sig(sig),
        )
    ...
    def test_to_bytes_unicode(self):
        self.assertEqual(
            b"\x07\xdcX \xa0\xe5\x18!]!,\xd5\x18\x8a\xe0,V5\xfa\xab",
            to_bytes("07dc5820a0e518215d212cd5188ae02c5635faab"),
        )
    ...
    def test_to_hexstring_bytes2(self):
        self.assertEqual(
            "07dc5820a0e518215d212cd5188ae02c5635faab",
    ...
    def test_to_hexstring_unicode(self):
        self.assertEqual(
            "07dc5820a0e518215d212cd5188ae02c5635faab",
            to_hexstring("07dc5820a0e518215d212cd5188ae02c5635faab"),
        )
    ...
    def test_der_encode_sig(self):
        ...
        der_sig = (
            "3045022100b2b31575f8536b284410d01217f688be3a9faf4ba0ba3a9093f983e40d630"
...
BECH32M_INVALID = [
    ...
    (
        ...
        "Character '111' not found in codebase",
    ),
    (
    ...
    (
        "BC1QR508D6QEJXTDG4Y5R3ZARVARYV98GJ9P",
        "Invalid decoded data length, must be 20 or 32 bytes",
    ),
    (
...
BECH32M_INVALID_PUBKEYHASH = [
    (
        "612079be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
        "Witness version must be between 0 and " "16",
    ),
    (
        "511979be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798",
        "Incorrect pubkeyhash length",
    ),
    ("0013751e76e8199196d454941c45d1b3a323f1433bd6", "Incorrect pubkeyhash length"),
    (
        "6614751e76e8199196d454941c45d1b3a323f1433bd6",
        "Witness version must be between 0 and 16",
    ),
]
...
class TestEncodingBech32SegwitAddresses(unittest.TestCase):
    ...
    def test_valid_checksum(self):
        ...
        for test in VALID_CHECKSUM:
            ...
            self.assertEqual(
                ...
                1,
                msg="Invalid checksum for address %s" % test,
            )
    ...
    def test_valid_address(self):
        ...
        for (address, hexscript) in VALID_ADDRESS:
            try:
                ...
                scriptpubkey = addr_bech32_to_pubkeyhash(
                    address, prefix="tb", include_witver=True
                )
    ...
    def test_quantity_class(self):
        self.assertEqual(
            str(Quantity(121608561109507200000, "H/s", precision=10)),
            "121.6085611095 EH/s",
        )
        self.assertEqual(
            str(Quantity(1 / 121608561109507200000, "ots", precision=10)),
            "8.2231052722 zots",
        )
        self.assertEqual(str(Quantity(0.0000000001, "m", precision=2)), "100.00 pm")
        self.assertEqual(
            str(Quantity(121608561109507200000000000000000)), "121608561.110 Y"
        )
        self.assertEqual(str(Quantity(1 / 1216085611095072000000000000000)), "0.000 y")
        self.assertEqual(
            str(Quantity(1 / 1216085611095072000000000000000, precision=10)),
            "0.0000008223 y",
        )
    ...
    def test_bech32m_valid(self):
        for addr, pubkeyhash in BECH32M_VALID:
            assert (
                pubkeyhash == addr_bech32_to_pubkeyhash(addr, include_witver=True).hex()
            )
            prefix = addr.split("1")[0].lower()
            witver = change_base(addr.split("1")[1][0], "bech32", 10)
            checksum_xor = addr_bech32_checksum(addr)
            addrc = pubkeyhash_to_addr_bech32(
                pubkeyhash, prefix, witver, checksum_xor=checksum_xor
            )
