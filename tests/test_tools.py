...
#    Unit Tests for fluxwallet Tools
...
class TestToolsCommandLineWallet(unittest.TestCase):
    def setUp(self):
        ...
        self.clw_executable = os.path.normpath(
            os.path.join(
                os.path.dirname(os.path.abspath(__file__)), "../fluxwallet/tools/clw.py"
            )
        )

    def test_tools_clw_create_wallet(self):
        cmd_wlt_create = (
            '%s %s test --passphrase "emotion camp sponsor curious bacon squeeze bean world '
    ...
    def test_tools_clw_create_multisig_wallet_one_key(self):
        ...
        output_wlt_create = (
            "if you understood and wrote down your key: Receive address:"
        )
    ...
    def test_tools_clw_create_multisig_wallet_error(self):
        ...
        output_wlt_create = (
            "Number of signatures required (second argument) must be a numeric value"
        )
        process = Popen(cmd_wlt_create, stdin=PIPE, stdout=PIPE, shell=True)
        poutput = process.communicate(input=b"y")
        self.assertIn(output_wlt_create, normalize_string(poutput[0]))
    ...
    def test_tools_clw_transaction_with_script(self):
        cmd_wlt_create = (
            '%s %s test2 --passphrase "emotion camp sponsor curious bacon squeeze bean world '
            'actual chicken obscure spray" -r -n fluxwallet_test -d %s'
            % (self.python_executable, self.clw_executable, self.DATABASE_URI)
        )
        ...
        cmd_wlt_transaction = (
            "%s %s test2 -d %s -t 21HVXMEdxdgjNzgfERhPwX4okXZ8WijHkvu 50000000 -f 100000 -p"
            % (self.python_executable, self.clw_executable, self.DATABASE_URI)
        )
    ...
    def test_tools_clw_create_litecoin_segwit_wallet(self):
        cmd_wlt_create = (
            '%s %s ltcsw --passphrase "lounge chief tip frog camera build trouble write end '
    ...
    def test_tools_clw_create_multisig_wallet_p2sh_segwit(self):
        ...
        cmd_wlt_create = (
            "%s %s testms-p2sh-segwit -m 3 2 %s -r -y p2sh-segwit -d %s"
            % (
                self.python_executable,
                self.clw_executable,
                " ".join(key_list),
