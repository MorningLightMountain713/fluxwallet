...
def read_config():
    ...
    if not config_file_name:
        ...
        if not BCL_CONFIG_FILE.exists():
            raise IOError(
                "Bitcoinlib configuration file not found: %s" % str(BCL_CONFIG_FILE)
            )
    data = config.read(str(BCL_CONFIG_FILE))
    BCL_DATA_DIR = Path(
        config_get("locations", "data_dir", fallback="~/.bitcoinlib")
    ).expanduser()
    # Database settings
    ...
    default_databasefile = DEFAULT_DATABASE = config_get(
        "locations", "default_databasefile", fallback="bitcoinlib.sqlite"
    )
    ...
    default_databasefile_cache = DEFAULT_DATABASE_CACHE = config_get(
        "locations", "default_databasefile_cache", fallback="bitcoinlib_cache.sqlite"
    )
    ...
    # Log settings
    ENABLE_BITCOINLIB_LOGGING = config_get(
        "logs", "enable_bitcoinlib_logging", fallback=True, is_boolean=True
    )
    BCL_LOG_FILE = Path(
        BCL_DATA_DIR, config_get("logs", "log_file", fallback="bitcoinlib.log")
    )
...
def initialize_lib():
    ...
    with instlogfile.open("w") as f:
        install_message = (
            "BitcoinLib installed, check further logs in bitcoinlib.log\n\n"
